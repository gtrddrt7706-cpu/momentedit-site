...
// ★리포트 모양(배열 · 또는 messageId/statusCode 가 있고 action 이 없는 객체)만 넘긴다 — GAS doPost 가 리포트로 읽는 모양과 같다.
...
const WAIT_MS = 4000;            // GAS 를 기다리는 한도 — 넘기면 이미 넘긴 것으로 친다(GAS 는 연결이 끊겨도 끝까지 돈다)
// ★[RELAY_WAIT 2026-09-25] 7초 → 4초. 솔라피가 답을 몇 초까지 기다리는지 모른다 — 실패 메일에 이유가 없고
//   developers.solapi.com 은 이 환경에서 막혀 있다(직접 시도함). 기다림이 솔라피 한도보다 길면 느린 GAS 한 번이
//   곧 실패 1회다(8회면 웹훅이 꺼진다). 끊어도 GAS 는 이미 받은 요청을 끝까지 처리하므로 짧게 끊는 쪽이 잃는 게 적다.
//   잃는 것은 «4초 뒤에 난 GAS 오류는 다시 받지 못한다» 하나 — handleSolapiReport 는 스스로 오류를 잡아 기록한다.
...
module.exports = async (req, res) => {
  ...
  const isReport = Array.isArray(parsed)
    ? parsed.length > 0
    : !!(parsed && typeof parsed === 'object' && (parsed.messageId || parsed.statusCode) && !parsed.action);
  if (!isReport) return out(400, { ok: false, error: 'not a report' });

  const ctl = new AbortController();
  const t = setTimeout(() => ctl.abort(), WAIT_MS);
  try {
    const r = await fetch(hook, { method: 'POST', headers: { 'content-type': 'application/json' }, body: raw, redirect: 'follow', signal: ctl.signal });
    let j = null;
