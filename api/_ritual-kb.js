...
function narrLines() {
  ...
  out.push(`- 기본값: 가족 코스는 가족 낭독, 그 밖은 나레이션(원천 COURSE_DEF). 고객이 언제든 바꿀 수 있음.`);
  ...
  out.push(`- (폐지) 축가 소개: "${D.NARR.song}" — 2026-08-09 폐지. 노래하고 싶은 두 분은 「자유 한 칸」에 넣는다. 묻더라도 권하지 말 것.`);
...
function minLines() {
  ...
  return [
    ...
    // [VEIL_RETIRED 2026-08-03] 베일 다운 폐지 — 전 예식 동시입장이라 실행 불가. 되살리지 말 것.
    `- 옵션 증감: 덕담 추가 +${dl.bless}분 · 축배+케이크 둘 다 +${dl.toastBoth}분 · 편지 양쪽(부모님+서로) +${dl.letterBoth}분 · 반지 빼면 ${dl.ringOff}분 · 첫인사 더하기 +${D.MIN.XM.welcome}분 · 편지 더하기 +${D.MIN.XM.letter}분`,   // [WINE_RETIRED] 밸리 · [SONG_RETIRED] 축가 뺌 — 고를 길이 없는 옵션의 분 수를 안내하면 그대로 오안내다
...
- 예식 형태: 하객 약 30명 안팎의 마이크로웨딩 · 140분 시그니처 시퀀스(준비 20분 → 단독 스냅 45분 → 본식 16~25분 → 인사 사진 30~39분 → 배웅 20분 · 본식이 짧은 코스를 고르면 그만큼 '인사 사진'이 늘어나 하루 140분은 그대로) 안에서 본식이 진행된다. 진행 안내는 성우 나레이션과 디렉터가 맡아 사회자 없이도 매끄럽다(나레이션은 진행 수단).
...
스텝: 시작 → 코스 선택 → 순간 고르기 → (순간들: 하객 맞이·입장·첫인사·서약·반지·성혼 선언·축배·편지·헌정·덕담·자유 한 칸) → 글 적어두기 → 완성.
★링 워밍은 2026-08-07 폐지됐다. 묻더라도 권하지 말 것.
★축가는 2026-08-09 폐지됐다(노래는 「자유 한 칸」으로). 와인 세리머니·케이크 커팅은 2026-08-16 폐지됐다(잔 드는 자리는 「축배 · 케이크」 하나). 묻더라도 권하지 말 것. [SONG_RETIRED][WINE_RETIRED]
...
${courseCount()}개 코스 중 하나를 고르고 순간을 조정해 예식 대본을 완성하는 화면이다. 하객 약 30명 안팎의 마이크로웨딩 · 본식 약 16~25분(이벤트 2~4개) · 진행 안내는 성우 나레이션과 디렉터가 맡는다(사회자 불필요).
