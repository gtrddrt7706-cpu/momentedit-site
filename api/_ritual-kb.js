...
function narrLines() {
  ...
  out.push('');
  out.push('[성혼 선언]');
  out.push(`- 낭독 주체 3종: 나레이션 / 하객 다 함께 합송 / 가족 낭독. 사회자(MC) 옵션은 없음.`);
  out.push(`- 기본값은 코스별로 다름: 가족 코스는 가족 낭독, 그 밖의 코스는 나레이션. 고객이 언제든 바꿀 수 있음.`);
  out.push(`- 선언 뒤에 하객분들이 한마디(네, 그러겠습니다)로 함께 답하는 순서를 둠. 외울 문장이 짧아 인쇄물이 필요 없음.`);
