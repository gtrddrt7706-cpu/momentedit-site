...
function narrLines() {
  ...
  out.push(`- 기본값은 코스별로 다름: 감동 코스는 하객 응답, 가족 코스는 가족 낭독, 그 밖의 코스는 나레이션. 고객이 언제든 바꿀 수 있음.`);
...
function minLines() {
  ...
  return [
    ...
    `- 본식은 **이벤트 2~4개 · 16~24분**이 기준이고 **상한은 25분**이다. 순간 하나에 평균 4분을 쓴다(반지는 2분, 편지는 6분 — 평균이지 각 순간의 길이가 아니다).
- 25분을 넘으면 밀도가 흐려지는 신호다. 사회자 없이 나레이션으로 이끄는 예식이라 길수록 루즈해진다.
- 본식이 짧아진 만큼 뒤의 「인사와 사진」이 길어진다(하루 140분은 그대로). 짧은 코스는 손해가 아니다.`,
  ].join('\n');
}
...
- 예식 형태: 하객 약 25명 안팎의 마이크로웨딩 · 140분 시그니처 시퀀스(준비 20분 → 단독 스냅 40분 → 본식 16~24분 → 인사와 사진 36~44분 → 배웅 20분 · 본식이 짧은 코스를 고르면 그만큼 '인사와 사진'이 늘어나 하루 140분은 그대로) 안에서 본식이 진행된다. 진행 안내는 성우 나레이션과 디렉터가 맡아 사회자 없이도 매끄럽다(나레이션은 진행 수단).
...
${courseCount()}개 코스 중 하나를 고르고 순간을 조정해 예식 대본을 완성하는 화면이다. 하객 약 25명 안팎의 마이크로웨딩 · 본식 약 16~24분(이벤트 2~4개) · 진행 안내는 성우 나레이션과 디렉터가 맡는다(사회자 불필요).
