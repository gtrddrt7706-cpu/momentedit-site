...
const MODEL = 'claude-sonnet-5';   // 예약·스케줄 접점(예비 고객) · Sonnet 5   // [MODEL_GEN5 2026-08-03] Sonnet 4.6 → Sonnet 5 세대 이전. 되돌리려면 git revert 1커밋.
...
// (2026-07-06 연구 반영) 예상 안 될 때의 억지 희소성은 무효+역효과(간파 시 신뢰↓·실패 시 경쟁사 이탈).
//   → 낮은 단계(0~1)는 희소성 금지. 실제로 일정이 차 있을 때(2~3)만 '사실'만 담백히. 경쟁·촉박 어휘 전면 금지.
const BUSY_LINE = {
  0: ' 혼잡·희소성 멘트 금지.',
  1: ' 혼잡·희소성 멘트 금지.',
  2: ' 실제로 이 시기 주말 일정이 어느 정도 채워졌을 때만 "이 시기는 주말 일정이 조금씩 채워지고 있어요" 같은 사실 한 줄 허용(촉박·경쟁·"서둘러" 어휘 금지).',
  3: ' "이 시기는 남은 일정이 많지 않아요"처럼 실제 마감 임박을 담백히 알리는 한 줄 권장(압박·경쟁 어휘 금지).',
};
...
3. 시간대는 [확인 결과]에 적힌 타임만 그대로 언급합니다(한 타임을 주면 한 타임, 여러 타임을 주면 그 타임들 그대로). [확인 결과]에 없는 시간대의 가능 여부는 말하지 않습니다.
...
9. 그 한 문장은 브랜드의 본질에서 꺼냅니다: 한 타임에 한 팀만 모시는 프라이빗함 · 가장 가까운 30명의 표정 · 140분의 또렷한 호흡 · 한 명의 디렉터가 처음부터 끝까지. 예: "이 타임은 그날 두 분과 초대하신 분들만을 위해 비워두는 시간이에요." / 가을 늦은 오후면 "빛이 가장 부드러운 시간이라 모두의 표정이 따뜻하게 남아요." 같은 멘트를 대화에서 반복하지 않습니다.
10. 사실이 아닌 주장은 절대 금지: 없는 문의·예약·마감을 있다고 하기, 수치·경쟁 상황 지어내기, [확인 결과]에 없는 가능 여부 말하기. 압박·과장 어투 금지. 사실은 [확인 결과]에 있는 것만.
11. (한가할 때의 신뢰 우선) 일정이 여유로워 희소성이 어울리지 않을 때는, 억지 혼잡 멘트로 채우지 말고 오히려 확답을 분명하게("네, 진행 가능한 일정으로 확인돼요") 주어 안심시킵니다. 필요하면 억지 희소성 대신 신뢰를 주는 담백한 한 줄만 곁들입니다: 한 타임에 한 팀만 모시는 프라이빗함, 실제 톤과 결과물을 인스타그램·홈페이지 아카이브에서 편히 확인하실 수 있다는 안내 등. 비어 있다는 인상을 주는 표현("요즘 한가해요", "언제든 다 돼요", "대부분 비어 있어요")과 전체 현황 나열은 여전히 금지입니다. 특정 날짜의 가능함을 분명히 확인해 주는 것과, 전체가 비어 보이게 하는 것은 다릅니다.`;
...
function decide(ex, taken, today, ctx, page, lv, consultDays) {
  ...
  if (ex.intent === 'open') {
    // (2026-07-06) 답답함 완화: 딱딱한 되물음 대신 따뜻하게 '대략적 시기'만 유도. 전체 현황·캘린더 비노출은 유지(빈 인상·경쟁사 보호).
    return '고객이 날짜 없이 전체 현황을 물었습니다. 전체 일정을 나열하거나 "많이 비어요·언제든 다 돼요" 같은 표현은 하지 말고, 딱딱하게 되묻지도 말고 따뜻하게: "대략 어느 계절이나 달 즈음 생각하고 계세요? 그 시기만 알려주시면 가능한 날을 바로 확인해 드릴게요"처럼 대략적인 시기를 편하게 여쭤보세요.';
  }
  ...
  if (!date && /^\d{4}-\d{2}-\d{2}$/.test(ex.periodFrom)) {
    ...
    // (2026-07-06 연구 반영: choice-overload) 후보를 1개가 아니라 최대 2개 제시 — 비교는 되되 과부하 없이.
    //   주말 우선(주말 요청이면 주말만). 주말/평일 혼용 방지: 첫 매치가 나온 pass에서 최대 2개만.
    const cands = [];
    for (const weekendPass of (ex.weekendOnly ? [true] : [true, false])) {
      let d = from;
      for (let i = 0; i < 124 && d <= to && cands.length < 2; i++) {
        const wd = dayOfWeek(d);
        const isWeekend = wd === 0 || wd === 6;
        if ((weekendPass ? isWeekend : !isWeekend) && freeSlot(taken, d, prefer) && cands.indexOf(d) === -1) cands.push(d);
        d = addDays(d, 1);
      }
      if (cands.length) break;   // 이 pass에서 하나라도 찾았으면 확정(주말+평일 섞지 않음)
    }
    const cand = cands[0] || '';
    if (!cand) {
      return '말씀하신 시기에는 안내 가능한 일정을 찾지 못했습니다. 다른 시기를 알려주시면 확인해 드리겠다고 안내하세요.' + (pol.busy >= 2 ? BUSY_LINE[3] : '');
    }
    // 니즈 먼저(사장 지시): 시간대를 안 정했으면 특정 시간을 찍지 말고, 후보 날짜(최대 2개)를 곁들여 희망 시간대를 먼저 여쭙니다.
    if (!prefer && !ex.anySlot) {
      const list = cands.map(function (c) { return koDate(c) + '(' + WD_KO[dayOfWeek(c)] + ')'; }).join(' 또는 ');
      return '고객이 말한 시기에서는 ' + list + ' 같은 날을 후보로 보고 있습니다. 특정 시간의 가능 여부는 아직 말하지 말고, "그 시기라면 ' + list + ' 같은 날이 있는데, 예식은 어느 시간대를 생각하고 계세요? 오전 9시 · 오후 12시 20분 · 늦은 오후 3시 40분 중에서 편하신 시간을 알려주시면 그 시간으로 확인해 드릴게요"처럼 후보 날짜를 곁들여 희망 시간대를 먼저 여쭤보세요. 위 후보 날짜만 말하고, 그 외 날짜의 가능 여부나 전체 현황은 말하지 마세요.';
    }
  ...
  if (!prefer && !ex.anySlot) {
    // 답답함 제거(2026-07-06): "세 타임 중 골라보세요"라고만 하지 말고, 그 날 '실제로 가능한 타임'을 먼저 알려준다.
    //   → 고객이 이미 찬 시간대를 골랐다가 "그건 마감"으로 되돌려받는 헛걸음을 없앤다. (그 날짜 하나만 · 전체 캘린더 비노출은 유지)
    const freeList = SLOTS.filter(function (s) { return (taken[date] || []).indexOf(s) === -1; });
    if (freeList.length) {
      const labels = freeList.map(function (s) { return SLOT_LABEL[s]; }).join(' · ');
      return '고객이 ' + koDate(date) + '(' + WD_KO[dayOfWeek(date)] + ')에 희망 시간대를 아직 말하지 않았습니다. 이 날은 ' + labels + ' 타임이 진행 가능한 일정으로 확인됩니다. 먼저 "네, ' + koDate(date) + '(' + WD_KO[dayOfWeek(date)] + ')은 ' + labels + ' 타임이 진행 가능한 일정으로 확인돼요"처럼 가능한 시간대를 함께 알려 안심시켜 드린 뒤, "그중 어느 시간대가 편하세요?"로 희망 시간을 여쭤보세요. 위에 적힌 이 타임들 외의 시간대나 다른 날짜의 가능 여부는 말하지 마세요.' + busyAt(date);
    }
  }
...
function okMsg(date, slot, note, page, cta) {
  ...
  // (2026-07-06 연구 반영) 희소성은 달력(주말·성수기) 기반 억지 허용이 아니라, 뒤에 붙는 busyAt(실제 예약율)만 권한을 갖는다.
  return note
    + '안내할 일정: ' + koDate(date) + '(' + WD_KO[wd] + ') ' + SLOT_LABEL[slot] + ' 타임 → 진행 가능한 일정으로 확인됨. 이 한 건만 안내하고,' + actionTxt(page, cta)
    + ' 혼잡·희소성 멘트는 뒤에 별도 허용 표기가 있을 때만 씁니다.';
}
function limitMsg(n) {
