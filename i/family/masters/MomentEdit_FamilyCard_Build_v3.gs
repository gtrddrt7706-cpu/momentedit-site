...
/**
 * ═══════════════════════════════════════════════════════════════════
 * Moment Edit · 가족 청첩장 빌드 시스템 (v4.0 옵션 C)
 * ═══════════════════════════════════════════════════════════════════
 *
 * 흐름:
...
const FC_DESIGN_CODES = ['01-classic', '02-editorial', '03-letterpress', '04-vermilion',
                       '05-botanical', '06-hangeul', '07-architect', '08-velour'];
...
function FC_buildFromSelectedRow() {
  ...
  if (row < 3) {
    SpreadsheetApp.getUi().alert('데이터 행을 선택해주세요 (3행 이상).');
    return;
  }
...
function FC_loadCoupleData(eventId) {
  ...
  for (let i = 2; i < values.length; i++) {  // 3행부터 데이터
