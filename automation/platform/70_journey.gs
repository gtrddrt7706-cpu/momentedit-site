...
function handleSignContract(body) {
  ...
  try {
    ...
    prev.계약 = {
      ...
      /* [CONTRACT_V111] 이 파일이 옛것이면 v1.11 문서(스냅 60 · 합 40)에 서명한 분이 'v1.10' 으로 찍혀, 나중에 옛 보존본(스냅 50 · 테이블 인사)을 보게 된다 */
      /* [CONTRACT_V110] 이 파일이 옛것이면 v1.10 문서에 서명한 분이 'v1.9' 로 찍혀, 나중에 옛 보존본(«성우 나레이션» · 케이크 줄 없음)을 보게 된다 */
...
//   index.html 가격표와 동일: 시그니처 평일 250만·주말/공휴일 330만, 웨딩스냅 60만.   ★인상 때 이 줄도 같이 고칠 것
/* ★[PRICE_2026_08 2026-08-14 사용자 결정] 주말 280→330만 · 평일 210→240만.
   ★여기가 **단일 출처**다 — 화면·계약서·AI 지식은 이 값을 옮겨 적은 사본이므로,
     바꿀 때는 automation/tests/merge-guard.sh 의 PRICE_2026_08 목록에 적힌 곳을 함께 본다.
   ★이미 계약한 분의 금액은 Customers 의 '계약총액'에 박혀 있어 여기 값이 바뀌어도 안 움직인다
     (이 표는 새 제안서를 만들 때만 쓰인다). 구가 서명자는 계약서 보존본 archive/v1-6.html 을 연다. */
var PRICING = {
  '시그니처': { 평일: 2500000, 주말: 3300000 },
  '웨딩스냅': 600000
};
