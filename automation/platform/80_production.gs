...
function handleSaveProductionTrack(body) {
  ...
  try {
    ...
    /* ★★[DONE_UNDO 2026-08-09] 아래 한 줄이 「처음부터 다시 만들기」를 무력화하고 있었다.
       ...
       ★어느 트랙에 실제로 닿는지 — 코드 세션 지적으로 바로잡는다(내 첫 주석이 틀렸다).
         `final`·`seat` 은 이 판정 **앞에서** 서버 정규화가 돌며 숫자 0 을 넣는다:
           final → {headcount:'', standing:0, extraFee:0}   ·   seat → {tables:[], note:'', _step:0}
         숫자 0 은 위 loop 의 어느 continue 에도 안 걸려 '안 비었다'가 된다 →
         **두 트랙은 종전대로 완료가 안 내려간다.** 실측으로 확인했다(정규화를 그대로 떼어 시험).
         안전한 쪽(안 내림)이라 버그는 아니지만, "다른 트랙도 같은 뜻"이라고 적었던 것은 사실이 아니었다.
         실제로 닿는 곳: ritual · dining · guideinfo · snap (정규화가 값을 안 심는 트랙들).
         지금 필요한 것은 ritual 하나뿐이라 두 트랙을 억지로 맞추지 않는다 —
