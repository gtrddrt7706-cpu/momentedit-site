...
var _CUST_PII_COLS = [
  ...
  // [PII_GAL 2026-07-26] 원본폴더ID = 원본링크에서 추출한 Drive 폴더 ID. 원본링크·보정본폴더를 비워도
  //   이 열이 남으면 같은 폴더를 그대로 가리키므로 파기가 반쪽이 된다(getResultGallery가 이 열로 열거).
  //   같은 등급의 열은 함께 비운다 — 파기 방향이라 되돌릴 수 없는 손실은 없다(파일 자체는 Drive에 그대로).
  '원본폴더ID'
];
