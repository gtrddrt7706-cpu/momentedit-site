  ...
  try {
    if (typeof deployFingerprint !== 'function') {
      ...
      if (!_rec) {
        ...
        /* ★[FP_BROAD 2026-09-06] 이 «같다»가 무엇을 근거로 한 말인지 밝힌다.
           지문이 표본 5개만 볼 때는, 그 밖의 파일만 고친 경우 재배포를 «안 해도» 같다고 나왔다(실측).
           지금은 전역 함수 전체의 이름:길이 서명이 함께 들어간다 — 그 함수 수를 찍어,
           서명이 실제로 작동하는지(0 이면 런타임이 열거를 못 한 것) 사람이 바로 알 수 있게 한다. */
        var _gs = (typeof _dsGlobalSig === 'function') ? String(_dsGlobalSig() || '') : '';
        var _gn = _gs ? _gs.split('#')[0] : '0';
        L.push(_gs
          ? '       (지문 범위: 핵심 5개 함수 + 전역 함수 ' + _gn + '개의 이름·길이 — 표본 밖 변경도 잡힌다)'
          : '       ★주의: 전역 서명이 비었습니다 — 지문이 핵심 5개 함수만 봅니다. 그 밖의 파일만 고쳤다면 이 «같다»는 재배포를 증명하지 못합니다.');
      } else {
        ...
        /* ★[STAMP_NEEDS_HIT 2026-09-19 실측] 이 줄이 «재배포하세요»만 말해서, 이미 재배포한 사람이
           같은 동작을 세 번 반복했다(15:36·15:41·15:51 — 지문이 세 번 다 똑같았다).
           원인은 이 판정의 «배포본» 쪽이 스크립트 속성이고, 그 값은 **배포된 코드가 /exec 를 탈 때**
           deployStamp() 가 쓴다는 데 있다. 재배포만으로는 갱신되지 않는다 — 요청이 한 번 들어와야 한다.
           위 «기록 없음» 갈래에는 그 안내가 있는데 여기만 없었다. 같은 안내를 여기에도 붙인다.
           ★순서를 함께 적는다 — 사람이 «재배포를 또 하라는 건가»로 읽지 않게. */
        chk('배포본이 지금 저장된 코드와 «다르다»', false,
          '아직 «새 버전»으로 배포하지 않았거나, 배포 뒤 사이트에 요청이 한 번도 안 들어왔습니다.\n'
          + '           → ①배포 관리에서 «새 버전»으로 재배포 ②관리자 페이지를 한 번 열기(그때 배포본 지문이 찍힙니다)\n'
          + '             ③이 점검 다시 실행.  ★②를 건너뛰면 재배포를 해도 이 줄이 그대로 나옵니다.\n'
          + '           (배포본 ' + _live + ' · 저장본 ' + _saved + ')');
      }
    }
