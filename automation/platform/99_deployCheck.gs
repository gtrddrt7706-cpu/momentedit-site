...
var FILES = [   /* 18개 — 86_dining_ai 제외(빈 슬롯) */
  ...
  L.push('');
  L.push('══ ①-B 파일 «안쪽»까지 다 붙었는가 (함수 전수 대조) ══');
  ...
  if (!REMOTE) {
    ...
    if (fnMiss === 0) chk('함수 ' + fnTotal + '개 전부 있음 (붙여넣다 잘린 파일 없음)', true);
  }
  ...
  L.push(badN === 0
    ...
    : '결과 — ★누락 ' + badN + '건. 각 MISS 줄의 «→» 가 할 일을 말합니다 (①~③ 은 그 파일을 다시 붙여넣고 «새 버전» 배포 · ④ 는 배포만 · ⑤ 는 함수 실행). (확인 ' + okN + '항목)');
