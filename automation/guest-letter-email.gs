...
function buildGuestLetterHtml(guestName, relation, recipient, message) {
  ...
  var rel = relation ? ('<span style="color:#9C9080;font-size:12px;"> · ' + esc(relation) + '</span>') : '';
  ...
  return '' +
    ...
    '<div style="color-scheme:dark;font-family:\'Noto Serif KR\',serif;max-width:560px;margin:0 auto;padding:46px 32px;background:#1E1A17;color:#E8E1D6;">' +
      ...
      '<div style="width:42px;height:1px;background:#C9A977;margin:24px auto 28px;"></div>' +
      '<p style="text-align:center;font-family:\'Cormorant Garamond\',serif;font-size:13px;letter-spacing:0.32em;color:#C9A977;margin:0;">A LETTER HAS ARRIVED</p>' +
      '<p style="text-align:center;font-size:15px;font-weight:300;color:#E8E1D6;margin:8px 0 0;">' + toTitle(recipient) + ' 편지가 도착했습니다</p>' +
      '<div style="background:#2A241F;border:1px solid rgba(255,255,255,0.08);border-radius:2px;padding:32px 28px;margin:26px 0;">' +
        '<div style="font-family:\'Cormorant Garamond\',serif;font-size:13px;letter-spacing:0.12em;color:#9C9080;margin-bottom:20px;">TO. ' + toShort(recipient) + '</div>' +
        '<div style="font-size:15px;line-height:2.05;font-weight:300;color:#E8E1D6;">' + body + '</div>' +
        '<div style="height:1px;background:rgba(255,255,255,0.08);margin:26px 0 16px;"></div>' +
        '<div style="text-align:right;font-size:13px;color:#B8AE9F;">From. ' + esc(guestName) + rel + '</div>' +
      '</div>' +
      '<div style="text-align:center;font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:11px;color:#C9A977;margin-top:30px;">Focus on the Essence, Record the Truth.</div>' +
      '<div style="text-align:center;font-size:10px;color:#7A7165;margin-top:12px;">Moment Edit · 두 분만을 위한 기록</div>' +
    '</div>' +
