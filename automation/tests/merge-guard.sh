...
# [CORE_PRICE_LINK 2026-08-16 사용자 결정] 「그 밖은 미리 밝힙니다」 → 「포함과 별도를 항목마다 적어
#   두었습니다」 + 금액을 가격 섹션(#invest)으로 가는 링크로. 약속만 하던 말이 '어디에 적혀 있는지'까지
#   말하게 됐다. ★금액을 카드에서 빼지 말 것 — 700px 위 「Hidden Costs」의 즉답 자리다(주석 CORE_PRICE_LINK).
