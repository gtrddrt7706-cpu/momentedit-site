 ...
 *  · 계좌 형식은 실제 Couples 시트를 그대로 따른다:
 *      - 본인 계좌 = 은행/계좌번호 분리 칸 (J groomBank·K groomAccount / L brideBank·M brideAccount)
 *      - 부모 계좌 = "은행 번호" 한 칸 (P~S groom/brideFather/MotherAccount)
 *    hydrate.coupleAccount(분리)·parseAccount(한 칸)가 각각 대응.
