...
// ─────────────────────── 배포 기록 (Deployment) ───────────────────────
// 2026-05-25 · 1차: createCoupleForm()로 폼+트리거 생성(구 18문항, 폼ID 1QOSFZ7...).
// 2026-05-25 · 2차(본 v3 개정): createCoupleForm()를 다시 실행해 새 폼 생성 후,
//             1차 구 폼은 삭제(스프레드시트에 두 폼이 연결되면 구 폼 제출이 오류 로그를 남김).
//             새 폼의 작성(viewform) 링크는 공개 저장소에 적지 않음(스팸 방지) — 비공개 보관.
