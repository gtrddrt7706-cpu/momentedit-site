...
(function () {
  ...
  var ROWS = [
    ...
    ['단독 스냅 촬영', '60분', ['09:20', '12:40', '16:00'], '하객과 분리된 캔들존·화이트존에서 두 분만의 화보 같은 스냅을 담아요. 캔들존 25분 · 이동 5분 · 화이트존 25분 · 입장 준비 5분이에요.'],   // [SNAP_50 2026-09-25] · [DAY_60 2026-09-26] 50 → 60
    ['하객 입장', '20분', ['10:00', '13:20', '16:40'], '하객분들이 입장하며 웰컴 음료와 좌석 안내를 받는 시간이에요. 본식 시작 4분 전에는 불을 낮추고 두 분의 영상을 틀어요.', true],   // [SNAP_50] 식전 영상은 늘 있다(사장님 결정 2)
    /* ★[TIME_HONEST] 시각이 길이와 맞지 않았다 — 2026-08-08 실측.
       옛 표: 입장 5분(10:00) + 본식 '30분'(10:05) + 단체 30분(10:30) + 배웅(11:00).
       10:05 + 30 = 10:35 인데 다음 줄이 10:30 이었다. 라벨만 30이고 시계는 25로 돌고 있었다.
       ★'신랑·신부 입장' 줄을 없애고 본식이 입장으로 시작하게 했다 — MIN.base(16~25)가 이미
         입장(entry)을 포함한 값이라, 따로 5분을 더하면 두 번 세는 것이다.
       ★★[DAY_PLAN 2026-08-09] 단독 스냅이 40 → 45분이 됐다(캔들존 20 · **이동 5** · 화이트존 20).
         이동 5분은 촬영을 쉬는 시간이 아니라 **문 여는 규칙**이다 — 하객이 캔들존에 들어오기 전에
         두 분이 비워야 마주치지 않는다(사용자 제안). 그만큼 본식이 10:00 → 10:05 로 밀리고
         본식+다함께 합이 60 → 55분이 됐다. 배웅 11:00 · 종료 11:20 은 그대로다.
       ★★[SNAP_50 2026-09-25] 스냅 50(입장 준비 5 더함) → 입장 09:50 · 본식 10:10 · 인사 사진 10:30. 아래는 그 전 판의 기록이다.
       ★하객 입장이 09:40 → 09:45 다. 본식 10:05 기준 여전히 **20분 전**이다 —
         청첩장의 '15분 전' 표기와 갈리므로 청첩장도 함께 고칠 것(같은 커밋에서 처리함).
         다 함께의 10:25 는 본식 20분(가운데) 기준 · 실제로는 10:21~10:29 사이에 시작한다. */
    /* ★★[DAY_60 · NO_TABLE_ROUND 2026-09-26 사장님 결정] 스냅 60 → 입장 10:00 · 본식 10:20 · 단체 사진 10:40(본식 20분 기준).
         «인사 사진»(자리마다 찾아가기)은 없앴다 — 인사는 문 앞 배웅에서 두 분이 한 분 한 분께. 되살리지 말 것.
         ★이 줄의 시각은 check-source-drift 가 ritual-data DAY 로 잰다([SEQ_ROW_CLOCK]). */
    ['본식', '8~25분쯤', ['10:20', '13:40', '17:00'], '두 분의 입장으로 시작해요. 담은 순간에 따라 길이가 달라지는 예식의 중심이에요. 순서는 저희가 맞춰요.'],   // [TIME_HONEST] · [SEQ_WORD 코워크 추가 점검 P3]
    ['단체 사진', '15~32분쯤', ['10:40', '14:00', '17:20'], '하객 모두와 한 장을 먼저 남기고, 이어서 가족 구도를 차례로 담아요. 본식이 짧아 시간이 남으면 두 분과 편하게 찍는 자유 사진이에요.'],   // [TIME_HONEST] · [GROUP_PHOTO]
    ['마무리', '20분', ['11:00', '14:20', '17:40'], '가족 · 친구와 남은 사진을 마치고, 두 분은 편하게 환복하며 마무리해요.'],   // [NO_TABLE_ROUND] · [PHOTO_THANKS 2026-09-26] 옛 «마무리·배웅 · 문 앞에서 한 분 한 분께» — 감사 인사는 단체 사진 안(전체 사진 바로 뒤)으로 옮겼다
  ];
  ...
  var SNAP_ROWS = [
    ...
    ['마무리·환복', '5~10분', '촬영을 마치고 편하게 환복하는 시간이에요.'],
  ];
  ...
  var css = ''
    ...
    + '.meseq-it::before{content:"";position:absolute;left:-6.5px;top:18px;width:9px;height:9px;border-radius:50%;background:var(--bg,#FAFAF8);border:1.5px solid var(--gold,#B89A75);box-sizing:content-box}'
    + '.meseq-it.hl::before{background:var(--seal,#6B2A24);border-color:var(--seal,#6B2A24)}'
    + '.meseq-clk{position:absolute;left:-64px;top:12px;width:52px;text-align:right;font-family:var(--serif-ko,serif);font-size:13px;font-weight:500;color:#4E3F31;letter-spacing:-0.02em;line-height:1.8}'
    + '.meseq-nm{font-family:var(--serif-ko,serif);font-size:14.5px;font-weight:500;color:var(--accent,#3A2D22);display:flex;align-items:baseline;gap:7px;flex-wrap:wrap;line-height:1.8}'
    ...
    + '.meseq-end{position:relative;padding:8px 0 2px 18px;font-family:var(--serif-ko,serif);font-size:12px;color:var(--light,#75705F);line-height:1.8}'
    + '.meseq-end::before{content:"";position:absolute;left:-5px;top:13px;width:9px;height:9px;border-radius:50%;background:var(--border,#DDD8D1)}'
    + '.meseq-end .meseq-clk{top:7px}'   // 마무리 행 시각을 텍스트 중심에 정렬(다른 행과 통일)
    ...
    + '.meseq-it.compact::before{top:16px}.meseq-it.compact .meseq-clk{top:10px}'
