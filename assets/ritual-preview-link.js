...
(function (w) {
  ...
  var KEYS = [
    ...
    /* ★[ENTRY_OUT_TONE 2026-08-11] 도착 직후 닫는 말은 **S.entry 를 그대로 따라간다** —
       전용 키를 두지 않는다. 처음엔 'entryOut' 이라는 덮어쓰기 문을 열어 뒀는데,
       [PREVIEW_KEYS] 가 「엔진은 읽고 KEYS 에도 있는데 **값을 만드는 곳이 없다**」로 막았다.
       digital 이 2026-08-02 에 냈던 사고와 같은 꼴이다 — 아무도 안 채우는 키는 문이 아니라 구멍이다.
       ★나중에 따로 고르게 하려면 **빌더에서 묻는 화면과 함께** 키를 되살릴 것. 키만 먼저 두지 말 것. */
