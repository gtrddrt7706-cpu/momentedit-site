...
(function (w) {
  ...
  var KEYS = [
    ...
    /* ★[ENTRY_OUT_PICK 2026-08-12 사용자 지시 "따로따로"] 도착 직후 닫는 말을 **따로 고른다.**
       ★이 키는 한 번 막혔던 키다. 2026-08-11 에 'entryOut' 을 먼저 열어 뒀다가
         [PREVIEW_KEYS] 가 「엔진은 읽고 KEYS 에도 있는데 **값을 만드는 곳이 없다**」로 막았다
         (digital 2026-08-02 사고와 같은 꼴 · 아무도 안 채우는 키는 문이 아니라 구멍이다).
         그때 「빌더에서 묻는 화면과 함께 되살릴 것」이라 적어 뒀고, 지금 그렇게 되살린다 —
         order-preview.html 의 [ENTRY_OUT_PICK] 칩이 **같은 커밋**에서 이 값을 만든다.
       ★값은 '' 또는 A~F 한 글자뿐이다. 글이 아니다 — 서약문·편지는 여기에도 주소에도 안 싣는다.
       ★빈값이면 종전대로 S.entry 를 따라간다(이미 저장해 둔 분들의 소리가 오늘 바뀌지 않게). */
    'entryOut',                                // 도착 직후 닫는 말(빈값 = 입장 느낌 따라가기)
    /* ★[PREVIEW_UPTO 2026-08-12] 어디까지 걸어왔는지 — 미리듣기가 그 뒤를 안 들려주려고 읽는다.
       ★값은 방문한 단계 **키 목록**뿐이다(off·ord 와 같은 꼴). 고객이 쓴 글은 여기에도 안 싣는다.
       ★키를 여는 커밋과 **값을 만드는 커밋이 같다**(order-preview 의 _embedSave).
         2026-08-11 에 entryOut 을 키만 먼저 열었다가 [PREVIEW_KEYS] 에 막혔다 — 같은 실수를 안 한다. */
    'seen',                                    // 방문한 단계 키 목록(빈 배열 = 끝까지 걸었음)
  ...
  var INJECT = ['digital', 'photoShare', 'meal', 'photoN', 'photoWishN'];   // [GROUP_PHOTO 2026-09-26] photoN(가족 구도 수)은 단체 사진 트랙 값 — 관리자 «당일 콘솔»이 얹는다   // [PHOTO_ASK] 사진 링크 유무 · [MEAL_GUIDE] 모일 식당 유무도 밖에서 받아 얹는다
