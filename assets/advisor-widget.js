...
(function () {
  ...
  var css = ''
    ...
    /* ★★[RAIL_OVERLAP_OK 2026-08-10 사용자 확인 · 2026-09-20 사장님 재확인] 이 레일이 본문 글자
       위에 겹치는 것은 **의도된 것이다.**
       > 사용자 원문: *"계획된거야"* (겹침 실측을 보고한 뒤의 답 · 2026-08-10)
       > 사장님 원문: *"우측 아이콘트랙은 손대지마 따로 여백을 두지도마 의도된거야"* (2026-09-20)
       ★고치지 말 것. 재는 사람마다 같은 결론에 닿아 같은 제안을 다시 들고 오게 되어 있다 —
         그러라고 여기 수치를 남긴다. 이미 두 세션이 각각 발견해 각각 고치려 했다.
       ★★그리고 2026-09-20 에 **네 번째로 되살아났다.** 이 주석은 멀쩡했고 merge-guard 의
         chk 도 그것을 지키고 있었는데도 그랬다. 원인이 드러났다 —
           ①종전 게이트는 «결정의 글»만 지켰다. 이 주석은 위젯 소스 안에 있고, 발견은
             index.html 을 «렌더해서 재는» 쪽에서 나온다. 재는 사람은 이 파일을 열 일이 없다.
           ②그리고 docs/handoff/design-round.md 가 §2-9 에서 「종결」이라 닫아 놓고
             세 줄 아래 §3 「다음 세션이 할 일」에서 **「다시 재라 · 선택지 3개를 내라」**고
             지시하고 있었다. round-protocol.md §6 은 제목이 아예 「보류」였다. 둘 다 닫았다.
         → 그래서 `scripts/audit/rail-overlap-intent.mjs` 를 만들었다. **극성이 거꾸로다** —
           겹침이 «사라지면» 붉는다. 누가 본문에 오른쪽 여백을 줘서 «고치면» 게이트가 잡는다.
       실측 (390px · 2026-08-10):
