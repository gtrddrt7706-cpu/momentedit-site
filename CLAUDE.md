...
| `checkRitualOrdOverride` | 80_production | (읽기 전용·진단) 저장된 식순 초안 중 `S.ord`가 배열인 레코드 세기 — 순서를 ↑↓로 직접 옮긴 고객. 코스 `seq`·`opt` 자리를 바꿀 때 그 변경이 기존 고객에게 닿는지를 가르는 유일한 변수다. **0건이면 재배열이 전원에게 적용**되고, 0이 아니면 그 고객은 **그대로 둔다**(고객이 직접 정한 순서라 되돌리는 게 사고). ★마이그레이션을 만들지 말 것. 아무것도 쓰지 않음 |
...
- **`chk` 자기참조 금지** (2026-07-31 코워크 합의): 대상 파일이 `merge-guard.sh` 자신이면 패턴을 앵커로 잡아 `chk` 줄이 매칭되지 않게 한다. `chk 'X' automation/tests/merge-guard.sh N`은 그 `chk` 줄 자체가 `X`를 포함하므로 **지키려는 대상이 마커 자신**이 된다 — 검사 블록을 통째로 지워도 통과한다. 개수로 상쇄(`N=2`)하는 것도 임시방편이라, 그 파일에 같은 문자열이 하나 더 생기는 날 조용히 무력해진다. 실행 줄만 잡는 앵커를 쓸 것 (예: `chk '^  _os=.*ritual-order-sim-audit' … 1` → 블록 삭제 시 `0<1`).
