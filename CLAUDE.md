...
**그래서 순서를 못 박는다 — PR 을 만든 «다음 도구 호출»이 `enable_pr_auto_merge`(SQUASH) 다.**
- 「unstable(검사가 아직 돌거나 실패 중)」로 거절당하면 **포기하지 말고**, 게이트가 끝난 뒤 한 번 더 건다.
  그래도 안 되면 그때 내가 직접 병합한다. 어느 쪽이든 사용자는 기다리지 않는다.
- 감시 타이머는 «auto-merge 를 못 건 경우»의 폴백이지 기본 경로가 아니다.
