...
**새 화면의 미감 방향**을 잡을 때만 쓴다. 기존 화면을 고칠 때는 쓰지 않는다 — 이 사이트의 방향은 이미 정해져 있다
(`../momentedit-design/SKILL.md` · 어긋나면 그쪽이 이긴다).
방향 자체는 우리와 가깝다 — 59행 «Spend your boldness in one place … keep everything around it quiet and disciplined».
다만 9행의 전제(«다른 누구와도 헷갈리지 않는 정체성을 새로 만든다»)는 이미 정체성이 있는 이 사이트에 맞지 않는다. 예외는 `../MOMENTEDIT_EXCEPTIONS.md`, 순서는 `CLAUDE.md` 의 `[DESIGN_SKILLS_12]`.
