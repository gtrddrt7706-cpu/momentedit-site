...
const POLITE = (s) => {
  ...
  /* ★★[NIKKA_TRAP 2026-09-23] 합쇼체 «의문형»이 통째로 빠져 있었다 — 「-ㅂ니까/-습니까」.
     실사고: 아버님 덕담 「그때 서준이가 뭐라고 했는지 **아십니까?**」가 **평대**로 세어져
     가짜 왕복이 두 번 더 잡혔고(전환 3회), 진짜 전환은 어머님과 같은 **1회**였다.
     그 바람에 멀쩡한 덕담을 고치러 갈 뻔했다 — 틀린 자로 재면 옳은 글을 고치게 된다([TIME_STAIR]).
     ★가르는 법은 「니다」와 같다: 합쇼체는 「니」 앞에 ㅂ 받침이 있다(아십니까·있습니까).
       「그러니까」의 「러」에는 받침이 없어 안 걸린다 — 연결어미와 갈린다. */
  if (/니까$/.test(t) && hasBieup(t[t.length - 3] || '')) return true;
