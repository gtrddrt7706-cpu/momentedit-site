...
/* ★[JOIN_GAP 2026-08-09 2차] 0.7 → 0.92. 사용자 실청: *"0.78 → 1초로 늘리자"*.
   ...
   ★★[확정 2026-08-09] 입장 6클립은 **0.78초 판으로 확정**됐다(사장님 실청 · 1.00초가 아니다).
     보정값은 구간마다 다르다(0.7→+0.08 · 0.9↑→+0.03) — 아래 GAP_RESIDUAL 은 0.7 부근의 값이라
     설정을 크게 올릴수록 환산이 어긋난다. 위 '1.00 을 들으려면 0.92' 는 그 전제 아래의 계산이다.
     ★확정 파일(05_entry-A ~ 10_entry-F)은 **재생성 금지**. 같은 설정으로 다시 만들어도 같은 소리가
       안 나온다 — 중간에 앞 무음(HEAD_LEADIN)이 바뀌어 지금 --gap 0.7 은 0.73 을 준다.
       소리는 설정이 아니라 **결과 파일**이 판정 대상이다(JOINED_MARK 커밋 참고).
       입장을 새로 녹음하는 날이 오기 전까지 이 여섯 개를 다시 만들지 말 것. */
