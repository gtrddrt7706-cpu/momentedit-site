...
//   합이 31.5초가 됐다. 지금 있는 어떤 나레이션보다 길었다(편지 제외).
//   ★한 번에 하나씩 옳은 문장을 더하면, 각각은 옳고 합은 틀린다. 더할 때마다 합을 봐야 한다.
//   그런데 그 '합'을 눈으로 재던 첫 시도에서 비교표가 통째로 틀렸다 —
//   "편지 빼면 폐식 23.3초가 최장"이라고 적었지만 23.3초는 축배였고 폐식은 13.2초,
//   진짜 최장은 declare-1-solemn 25.9초였다. 그래서 사람이 세지 않게 여기서 센다.
//
// 재는 식은 조립기(assemble-narration.mjs)와 같다 — 자가 둘이면 둘이 다른 말을 하는 날이 온다.
//   길이 = head + tail + Σ(before + after + 음절/300*60)
...
console.log(`클립 ${rows.length}개 · 중앙값 ${median.toFixed(1)}초 · 편지 제외 최장 ${noLetter[0].f} ${noLetter[0].sec.toFixed(1)}초\n`);
console.log('긴 순 상위 8');
...
if (!ro) no('narr-round-open 을 manifest 에서 못 찾았습니다 — 이 검사도 같은 커밋에서 고치세요');
  ...
  else console.log(`\nok narr-round-open ${ro.sec.toFixed(1)}초 · 전체 ${rank}위/${rows.length} · 주석 수치와 일치`);
  ...
  else console.log(`ok 편지 제외 최장 declare-1-solemn ${noLetter[0].sec.toFixed(1)}초 — 주석과 일치`);
