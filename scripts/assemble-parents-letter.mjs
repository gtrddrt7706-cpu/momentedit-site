...
/* parents.html은 번호 없는 경로를 부른다 — 사본을 둔다 (★PARENTS_DUAL_PATH)
   ...
   ★★★[LETTER_AUDIO_UNIFIED 2026-09-19] 위 「갈라진 채로 둔다」(2026-08-21)는 **거두어졌다.**
     사장님: *"화면이랑 재생파일이 동일하게"* — 그때의 전제가 사라졌기 때문이다.
     대본이 B판으로 바뀌면서(PARENTS_B · PAR_PYEBAEK_OUT) 예식 당일 클립은 어느 글과도 안 맞게 됐다.
     ★이제 두 경로는 **바이트까지 같다.** 게이트가 cmp 로 그것을 본다.
     ★[LETTER_AUDIO_UNIFIED] 다시 가르지 말 것 — 가르려면 사장님이 먼저 말씀하신다.
       (2026-08-21 의 규칙을 지우는 것이 아니라 방향만 뒤집은 것이다. 옛 기록은 위에 남긴다.)
