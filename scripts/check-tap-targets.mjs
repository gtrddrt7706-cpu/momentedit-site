...
//   실측(e1cbbc1): .github/workflows 중 검사를 도는 것은 merge-guard.yml 하나이고,
//   그 merge-guard.sh 는 이 파일을 **grep 으로 마커만 센다**(969행이 스스로 적어 두었다).
//   deploy.sh 안 check- 호출 0건 · 사용자 훅 0개 · 저장소 전체에서 실행 호출 0건.
//   ★내가 「종료 코드 2 도 0 이 아니라 게이트가 그대로 막는다」고 적었던 것은 **틀렸다.**
//     막는 힘이 안 줄어든 건 맞지만 **원래 0 이라서** 참이었다. 없는 방벽을 근거로 안심하면
//     다음 사람이 "게이트가 본다"고 믿고 손검사를 건너뛴다.
//   ★게이트에 넣지 않는 것은 판단이다 — 브라우저와 로컬 서버가 필요해 CI 에서 느리고 잘 운다.
//     넣지 않는 대신 **자가 어디까지 닿는지 여기 적는다.** [TAP_UNSEEN] 이 '못 잰 것'을
//     화면에 적는 것과 같은 성격이다.
//
