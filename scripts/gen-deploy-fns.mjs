...
/* ★★[STAMP_FORCE 2026-09-22] --stamp 면 «내용이 그대로여도» 날짜를 새로 찍는다.
   왜 필요한가 — [LIST_AGE] 는 `_생성` 을 «가장 최근 .gs 커밋 날짜»와 견준다. 그런데 스쿼시 병합은
   내용을 하나도 안 바꾸면서 **커밋 날짜만 앞으로 옮긴다.** 병합이 자정을 넘기면 그날 main 은
   내용상 멀쩡한데 [LIST_AGE] 가 빨개지고, 생성기를 돌려도 `same` 이라 날짜가 안 바뀐다.
   바로 위 MARKS_STAMP 주석이 경고한 «처방이 듣지 않는 빨강»이 다른 경로로 또 생긴 것이다 —
   그 상태는 사람에게 «자동생성 파일을 손으로 고쳐라»를 시킨다(집 규칙이 금지한 짓이다).
   ★검사를 무르게 하지 않는다. 날짜를 새로 찍는 것은 사실을 적는 일이다 — 그 목록은 정말로
     그 시각의 저장소를 읽어 만든 것이다. 다만 «돌릴 때마다 바뀌면 커밋이 지저분해진다»는
     아래 원칙은 지킨다 — 그래서 자동이 아니라 --stamp 를 준 때만 찍는다. */
