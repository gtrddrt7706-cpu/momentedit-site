 ...
 *   덤으로 표의 크기가 곧 새 클립 수다 — 19 이 아니면 뭔가 빠진 것이다.
 *
 * ★★[TONE_CULL 2026-09-20 사장님 「겹치거나 별로인 거 전부 삭제해 과감하게 갯수 상관없이」]
 *   38 → **19**. 코워크가 50벌을 문면으로 읽어 열여덟을 버렸다(숫자로 고른 것은 하나도 없다).
...
// 표의 크기 = 새로 나는 클립 수. 52 §3 은 +38 이었고, [TONE_CULL]+[CULL_2]+[CULL_3] 로 19 가 됐다.
let clips = 0;
...
console.log(`문안 ${n}벌 · 클립 ${clips}개 ([TONE_CULL] 뒤 기준 19)`);
console.log((same ? 'ok   ' : 'FAIL ') + '문서(21_B §3)와 ritual-data.js 의 TONE 이 같다');
if (clips !== 19) console.log(`FAIL 클립 수가 19 이 아니다 — 문서에서 빠졌거나 더 들어왔다 (지금 ${clips})`);
if (!same) console.log('   → node scripts/build-tone-table.mjs --write 로 다시 뽑을 것');
process.exit((same && clips === 19) ? 0 : 1);
