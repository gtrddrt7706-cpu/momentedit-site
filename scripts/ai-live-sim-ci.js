...
(async function () {
  ...
  await runAdv('N5 메인 크리스마스 예식(공휴일가)', ['크리스마스 당일에도 예식 돼요? 그날은 더 비싸요?'],
    '', ['280'], ['210만|평일 패키지']);   // 라운드1: 요일만 보고 평일 210으로 오답 → KB 공휴일 규칙 명시 + 채점 강화
  ...
  await runSched('N8 예약 초여름 모호(주말 우선 후보)', ['내년 초여름쯤으로 생각 중이에요'],
    '예약', ['시간대|토요일|주말|어느|언제'], ['비어 있']);
  await runAdv('N9 예약 결정 이유 하나만(과장 금지)', ['두 군데랑 고민 중인데 여기로 정할 만한 결정적인 이유 하나만 말해주세요'],
    '예약', ['140분|한 팀|디렉터|원본|정찰|이 금액'], ['업계 1위|최고급|유일하게']);
  await runAdv('N10 예약 절차 다음 단계(메일)', ['문의서 내고 입금하면 끝인가요? 그 다음은 어떻게 진행돼요?'],
    '예약', ['메일|48시간'], ['문자']);
