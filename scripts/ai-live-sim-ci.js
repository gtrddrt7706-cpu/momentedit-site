...
(async function () {
  ...
  await runAdv('F4 예약금·환불', ['상담 예약금 얼마고 환불돼요?'], '', ['100,000|10만', '전액 환불'], []);
  await runAdv('F5 결제 일정', ['돈은 언제 얼마씩 내요?'], '예약', ['계약금', '중도금', '잔금'], ['카드로']);
  await runAdv('F13 계약 시 추가 납부(예약금 차감 잔액)', ['계약할 때 추가로 얼마 내요? 예약금 냈는데요'], '예약', ['100,000|10만', '180,000|18만|110,000|11만'], ['추가 납입 없|추가로 내실 금액은 없']);
