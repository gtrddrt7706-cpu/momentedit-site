...
(async function () {
  ...
  await runSched('S6 무관 질문(전용 창구 안내)', ['스드메 가격이 얼마예요?'], '예약', ['예식일'], ['330만', '240만']);
  ...
  await runAdv('F13 계약 시 추가 납부(예약금 차감 잔액)', ['계약할 때 추가로 얼마 내요? 예약금 냈는데요'], '예약', ['100,000|10만', '230,000|23만|140,000|14만'], ['추가 납입 없|추가로 내실 금액은 없', '230만|140만']);   // 풀런2 실사례: 18만을 "180만원"으로 자릿수 오기(PRICE_2026_08 이후는 23만/14만) → 단정 감시('270만'은 총액-예약금 합산 설명으로 정답 가능해 제외)
  ...
  await runAdv('E5 메인 영어 가격', ['How much does the all-in-one package cost?'], '', ['330|240|3,300,000|2,400,000'], []);
  ...
  await runSched('E14 스케줄 가격 거부(전용창구)', ['여기 패키지 가격 얼마예요?'], '스케줄', ['예식일|예식 날짜'], ['330만', '240만']);   // 풀런1 오탐 보정: "마음에 두신 예식 날짜" 표현 인정
  ...
  await runAdv('N5 메인 크리스마스 예식(공휴일가)', ['크리스마스 당일에도 예식 돼요? 그날은 더 비싸요?'],
    '', ['330'], ['(이므로|적용되어|적용돼|당일은) 240만']);   // 라운드4 오탐 보정: 대조 설명("평일이면 240")은 허용, 크리스마스에 240 적용 단정만 금지
