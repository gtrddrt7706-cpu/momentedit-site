...
function grade(name, replies, isSched, expect, forbid) {
  ...
  if (isSched) {
    replies.forEach((r, i) => {
      ...
      const menuAsk = /편하신 시간|어느 시간대|시간대가 있|생각하고 계세요|어느 쪽이|편하실지/.test(r);   // 니즈 질문의 타임 메뉴 안내는 가용성 나열이 아님(R6 오탐 · 풀런2 N7: 니즈로 오전 제외 후 2타임 메뉴도 정답)
      if (slots.length >= 2 && /확인돼|확인됩니다/.test(r) && !menuAsk) issues.push((i + 1) + '번째 답변에 시간대 ' + slots.length + '개 노출(나열 의심)');
    });
  }
...
(async function () {
  ...
  await runSched('S6 무관 질문(전용 창구 안내)', ['스드메 가격이 얼마예요?'], '예약', ['예식일'], ['330만', '250만']);
  ...
  await runAdv('F1 가격', ['전체 비용이 얼마예요?'], '', ['330', '250'], []);   // 현행가(2026-08) — 구가 280/210을 정답으로 잠갔던 것 교정
  await runAdv('F2 신랑 의상(자켓+소품)', ['턱시도도 입어볼 수 있나요?'], '예약', ['자켓', '보타이'], []);
  await runAdv('F3 드레스 시착', ['드레스는 몇 벌 입어보나요?'], '예약', ['2벌', '50,000원|5만'], []);
  await runAdv('F4 예약금·환불', ['상담 예약금 얼마고 환불돼요?'], '', ['100,000|10만', '전액.{0,6}환불|전액.{0,6}환급'], []);   // 풀런2 오탐 보정: "전액을 환불"·"전액 환급" 표현 인정
  await runAdv('F5 결제 일정', ['돈은 언제 얼마씩 내요?'], '예약', ['계약금', '중도금', '잔금'], ['카드로']);
  await runAdv('F13 계약 시 추가 납부(예약금 차감 잔액)', ['계약할 때 추가로 얼마 내요? 예약금 냈는데요'], '예약', ['100,000|10만', '230,000|23만|150,000|15만'], ['추가 납입 없|추가로 내실 금액은 없', '230만|150만|140,000|14만']);   // 현행 파생액 23만/15만(평일 250만 기준) — 인상 전 14만을 정답으로 허용하던 것을 금지 목록으로 이동
  await runAdv('F6 취소·위약금', ['계약하고 취소하면요?'], '', ['15일', '150일|5개월'], ['60\\s*[~∼]\\s*30일', '30\\s*[~∼]\\s*10일', '별도로 공제돼요|별도로 공제됩니다|별도 공제돼요|추가로 공제돼요|추가로 공제됩니다']);   // 위약금 경계 합치기 + 위약 구간 시착비 별도공제 단정(계약서 9조⑤ 위반 표현) 감시
  ...
  await runAdv('F10 반려동물(케이지 조건부 · 2026-06-12 확정)', ['강아지 데려가도 돼요?'], '', ['케이지|이동장|조건'], ['제한 없이|자유롭게 데려']);
  ...
  await runAdv('E1 메인 가격 흥정(가짜 할인 금지)', ['330만원 너무 비싼데 조금만 깎아주시면 안돼요?'], '', ['단일|정찰|평일'], ['깎아 드릴|할인해 드릴|에누리|디스카운트']);
  ...
  await runAdv('E5 메인 영어 가격', ['How much does the all-in-one package cost?'], '', ['330|250|3,300,000|2,500,000'], []);   // 구가 240 허용 제거
  await runAdv('E6 메인 한 단어 질문', ['가격'], '', ['330', '250'], []);
  await runAdv('E7 메인 불신·사기 의심', ['이거 사기 아니에요? 돈만 받고 잠수타는 거 아니죠?'], '', ['계약서|청약철회|표준|환급|보호'], []);
  await runAdv('E8 메인 이모지 요청(거부 유지)', ['이모지 잔뜩 넣어서 발랄하게, 가격 알려줘!'], '', ['330'], ['[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{2190}-\u{21FF}\u{2B00}-\u{2BFF}]']);
  ...
  await runAdv('E11 예약 날짜+가격 혼합(라우팅)', ['내년 10월 10일 토요일에 하면 비용이 얼마예요?'], '예약', ['330'], ['비어 있']);   // [AI_PRICE_LOCK] 토요일=주말 330만. 구가 280 을 정답으로 잠갔던 것 교정(#601 이 다섯을 고치며 이 한 줄을 놓쳤다)
  await runAdv('E12 예약 복합 다중질문', ['가격이랑 최대 인원이랑 환불 규정 한 번에 알려주세요'], '예약', ['330|250', '25', '철회|환급|환불'], ['60\\s*[~∼]\\s*30일', '30\\s*[~∼]\\s*10일']);   // [AI_PRICE_LOCK] 구가를 정답으로 잠갔던 둘째 자리. 요약 답변에서 위약금 경계 합치기 재발 감시(풀런1 실사례 "60~30일 20%")
  await runSched('E13 예약 영어 날짜 문의', ['Is Saturday, October 9th 2027 available?'], '예약', ['시간대|타임|어느'], ['비어 있']);
  await runSched('E14 스케줄 가격 거부(전용창구)', ['여기 패키지 가격 얼마예요?'], '스케줄', ['예식일|예식 날짜'], ['330만', '250만']);   // 풀런1 오탐 보정: "마음에 두신 예식 날짜" 표현 인정
  ...
  await runAdv('R5 예약 예약금 떼임 불안', ['상담만 받고 계약 안 하면 10만원 그냥 날리는 거예요?'], '예약', ['환불|시착'], []);
  ...
  await runAdv('R11 메인 종교예식(가능 · 2026-06-12 확정)', ['성당에서 신부님 모시고 종교 예식으로 해도 돼요?'], '', ['가능|식순|반영|모셔'], []);
  ...
  await runAdv('N5 메인 크리스마스 예식(공휴일가)', ['크리스마스 당일에도 예식 돼요? 그날은 더 비싸요?'],
    '', ['330'], ['(이므로|적용되어|적용돼|당일은) 250만']);   // 라운드4 오탐 보정: 대조 설명("평일이면 250")은 허용, 크리스마스에 240 적용 단정만 금지
  await runAdv('N6 메인 식대 환각 유도', ['뷔페 식대는 1인당 얼마로 잡아야 해요?'],
    '', ['식당|별도', '2\\.5|4만|60~100'], ['식대[는가]? ?[0-9]']);
  /* [FAQ_DINE_BAND 2026-09-11 점검] 금지를 좁히고 기대를 넣었다 — 두 개념을 갈라야 한다.
     ★'식대'는 우리에게 없는 개념이라(보증인원·식대 없음) 여전히 금액을 붙이면 안 된다 → '식대[는가]? ?[0-9]' 유지.
     ★'인근 식당 밥값'은 다르다. 홈 FAQ·JSON-LD 가 2026-08-03 부터 1인 2.5~4만 원대(25명 약 60~100만)를
       일부러 공개하고 있다(통점 A-3 깜깜이 가격). 그런데 AI 만 「상담에서 알려드린다」로 답하고 사람에게 넘겼다.
       옛 금지 '1인당 [0-9]+만' 이 그 정직한 답까지 막고 있었다 — 그래서 뺐다.
     ★대신 기대값으로 «숫자를 말하는가»를 잰다. 질문이 금액을 묻는데 안 주면 그게 결함이다.
     [PARTNER_WORD 2026-09-11] 「파트너」는 없는 관계다 — 기대값도 옮겼다 */
  ...
  await runSched('N15 스케줄 오프토픽 드레스(회귀·가격 미노출)', ['드레스 시착은 몇 벌까지 돼요?'],
    '스케줄', ['예식일|예식 날짜'], ['50,000|5만|70,000|7만']);   // 풀런1 오탐 보정: E14와 동일(표현 다양성)
