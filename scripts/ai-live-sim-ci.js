...
(async function () {
  ...
  await runAdv('F13 계약 시 추가 납부(예약금 차감 잔액)', ['계약할 때 추가로 얼마 내요? 예약금 냈는데요'], '예약', ['100,000|10만', '180,000|18만|110,000|11만'], ['추가 납입 없|추가로 내실 금액은 없', '180만|110만']);   // 풀런2 실사례: 18만을 "180만원"으로 자릿수 오기 → 단정 감시('270만'은 총액-예약금 합산 설명으로 정답 가능해 제외)
  ...
  await runSched('E14 스케줄 가격 거부(전용창구)', ['여기 패키지 가격 얼마예요?'], '스케줄', ['예식일|예식 날짜'], ['280만', '210만']);   // 풀런1 오탐 보정: "마음에 두신 예식 날짜" 표현 인정
