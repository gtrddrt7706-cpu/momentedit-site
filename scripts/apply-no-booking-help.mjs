...
const E = [
  ...
  ['mypage.html', 1,
   ...
   ''],
  /* 2026-09-26 사장님 결정 ⑰(mypage DN_TEL_MAP)이 이 자리의 «나중 결정»이다 — 「확정되는 대로 안내해 드릴게요」도
     누가 언제 알려 주는지가 없어 «지도에서 매장 번호를 확인해 예약해 주세요»로 바뀌었다.
     옛 목표 문장을 그대로 두면 이 스크립트를 다시 돌릴 때(ALREADY_DONE 경로) 옛 문구 0개 · 목표 문구 0개로 «틀림»이 되어
     나머지 자리까지 아무것도 못 쓴다. 그래서 목표를 지금 문장으로 옮겼다(«디렉터에게 요청» 을 걷어내는 이 스크립트의 뜻은 같다). */
  ['mypage.html', 1,
   '전화번호는 확정되는 대로 안내해 드릴게요. 디렉터에게 요청하셔도 돼요.',
   '지도에서 매장 번호를 확인해 예약해 주세요.'],
