...
const said = [];
/* ★sink 를 받는 이유 — 아래 [REASON_GAP] 이 **같은 걸러내기**로 근거 덩이를 따로 재야 한다.
   자를 하나 더 만들면 그 자가 또 어긋난다(이 저장소가 반복해 겪은 것). 자는 하나만 둔다. */
const grab = (src, where, sink = said) => {
  ...
  for (const m of src.matchAll(/(폰|PC)\s*(?<![\d.])(\d{1,3})(?![\d.])(?:\s*px)?/g)) {
    ...
    sink.push({ who: m[1], n: +m[2], where, at: m[0] });
  }
};
       ...
       주석 안에 주석 닫는 기호를 넣지 말 것 — 예시는 가운뎃점으로 바꿔 적는다. */
{
  ...
  if (!long_.length) {
    ...
    /* ★★[REASON_GAP 2026-08-10 · 코드 세션 실측] '폰·PC 숫자가 있는가'로는 아직 모자랐다.
       실측: 근거 덩이를 채움글 + 「워드마크가 폰 32px · PC 50px 이다」로 갈아 끼우니 **exit 0** 이었다.
       그 숫자는 **간격이 아니라 글자 크기**다. 게다가 위 grab 이 크기를 걸러 said 에도 안 들어가니
       불일치도 안 난다 — 즉 다른 것의 치수를 인용하는 것만으로 '근거가 있다'가 됐다.
       ★대용품을 한 겹 올렸을 뿐이었다: 길이 → 숫자 유무. 여기서 **간격값 자체**를 요구해 끊는다.
       ★같은 걸러내기(grab)를 그대로 쓴다. 자를 새로 만들면 그 자가 또 어긋난다. */
    const fromReason = [];
    for (const b of long_) grab(b, '근거 덩이', fromReason);
    if (!fromReason.some((x) => x.who === '폰') || !fromReason.some((x) => x.who === 'PC')) {
      no('근거 덩이가 말하는 폰·PC 숫자가 **글자 크기뿐**입니다 — 간격값(폰·PC 사이)을 적어야 근거입니다');
    }
  }
