...
/* ─────────────────────────────────────────────────────────────────
   ...
   ★왜 여기인가 — 이 파일이 잡는 병과 정확히 같다. 원천(항목 수·기간)이 어딘가에
     손으로 또 적혔고, 원천이 자라는 동안 사본만 늙었다.
   ★실사고: `docs/검사가_속인_방식_목록.md` 의 제목이 「열두 줄」인 채 항목이 열일곱이 됐고,
     머리말이 「08-09~10 이틀」인 채 08-08 · 08-11 사고가 들어왔다.
     그 문서가 말하는 병을 그 문서의 첫 두 줄이 앓고 있었다. 열다섯 항목을 쓰는 동안
     아무도 첫 줄을 세어 보지 않았다.
   ★고침이 「17 로 고치기」가 아니었던 것이 요점이다 — 그러면 ★16 을 쓰는 날 또 낡는다.
     **세지 않아도 참인 문장**으로 바꾸는 것이 고침이다. 사본을 하나 줄인다.
   ★그런데 그 규칙이 「사람이 지킨다」로 남아 있었다(문서 안 당부 한 줄).
     이 저장소는 사람이 기억해야 하는 구조를 안 남기기로 했다. 그래서 기계가 지킨다.

   무엇을 보나 — **머리(첫 `## ` 앞)만** 본다. 본문의 「실사고 셋」·「돌연변이 여덟 발」은
   그 항목 안에서만 참이면 되는 국소 수치라 늙지 않는다. 늙는 것은 **목록 전체를 센 수**다.
   ★HTML 주석은 벗기고 본다 — 사고 경위를 적은 주석에 옛 문구(「열두 줄」)가 그대로 있고,
     그걸 세면 경위를 적었다는 이유로 붉어진다(주석이 검사를 대신 맞는 병 · 이 저장소 단골).
   ───────────────────────────────────────────────────────────────── */
{
  const DOC = 'docs/검사가_속인_방식_목록.md';
  const p = path.join(root, DOC);
  if (!fs.existsSync(p)) no(`${DOC} 가 없다 — 교훈 목록이 사라지면 이 검사도 뜻이 없다`);
  else {
    const raw = fs.readFileSync(p, 'utf8');
    /* ★[HEAD_BOUNDED 2026-08-11 · 코워크 적대검증] 머리를 **못 정하면 그렇다고 말한다.**
       옛 판은 `## ` 가 하나도 없으면 머리 = 문서 전체가 됐다. 「더 넓게 보니 안전한 방향」이라
       적혀 있었는데 실측은 반대였다 — 모든 `## ` 를 `# ` 로 바꿔 재니 **헛붉음 아홉 건**이 났다:
         3건 · 3줄 · 0개 · 0건 · 한 줄 · 100개 · 99개 · 가지 하나 · 7일간
       전부 본문의 멀쩡한 글이다(★7 의 「3건 붉었다」, ★13·14·15 의 「한 줄:」…).
       ★넓게 보는 것은 안전하지 않다. 이 검사의 값은 **머리만** 본다는 좁힘에 있고,
         그 좁힘이 사라지면 늑대를 아홉 번 부른다(★9). 그러면 다음 진짜 rot 때 아무도 안 본다.
       ★그래서 못 정할 때는 「전부 본다」가 아니라 **「못 정한다」**고 붉게 선다. */
    const hasSection = /^## /m.test(raw);
    if (!hasSection) {
      no(`${DOC} 에 \`## \` 절 제목이 하나도 없다 — 머리 구간을 정할 수 없다.\n`
        + '    문서 전체를 머리로 보면 본문의 멀쩡한 수까지 붉어진다(실측 9건 · HEAD_BOUNDED).\n'
        + '    절 제목을 `## ` 로 되돌리거나, 이 검사의 머리 판정을 새 구조에 맞게 고칠 것.');
    }
    const head = raw.split(/^## /m)[0].replace(/<!--[\s\S]*?-->/g, '');   // 주석 벗긴 머리
    /* ★★[COUNT_NET_OPEN 2026-08-11 · 클로드코드 적대검증] 세는 말의 **목록**을 버리고
         「머리말에는 수가 없어야 한다」로 뒤집는다.

       옛 판은 수사(열두·17…)와 세는 이름(줄·항목·가지)을 **목록으로** 들고 있었다.
       그 목록에 없는 표기를 열한 발 쏴 보니 **일곱이 샜다**(실측):
         seventeen items · 17 entries · 열일곱 개의 항목 · 항목 17개 · XVII 줄 · 마흔 줄 · 백 줄 · 17건
       ★이건 ★15 가 이미 이름 붙인 병이다 — 「목록형 그물은 새 표기가 생기면 샌다」.
         그 문서를 지키는 검사가 그 문서의 교훈을 어기고 있었다. 그물 2 를 그물 3 으로 바꾼다.

       ★그물의 두 쪽은 성격이 다르다 — 이 구분이 요점이다(★15 의 그물2/그물3 과 같은 갈래).
         **수 쪽은 목록이면 안 된다.** 표기가 끝없이 늘어난다(17 · 열일곱 · seventeen · XVII · 마흔 · 백).
           → 일반적으로 짠다: 숫자든 한글 수사든 영문이든 로마숫자든 「수」이기만 하면 잡힌다.
         **세는 이름 쪽은 목록이어도 된다.** 「목록의 한 항목」을 부르는 말은 뜻이 정해져 있어
           많아야 열 몇 개다(줄·항목·가지·개·건·교훈·items·entries…). 그리고 이 쪽이
           **의미를 판별한다** — 수만으로는 「쓸모는 하나다」·「실사고가 하나씩」 같은
           멀쩡한 글까지 붉어진다(수 전면 금지로 짜 봤다가 실제로 그렇게 됐다).
       ★그래서 「수 + 세는 이름」이 **붙어 있을 때**만 잡는다. 순서는 양쪽 다 본다
         (「17항목」·「항목 17개」). 사이가 벌어지면 세는 말이 아니라 문장이다. */
    const SAFE = [
      /\d{4}-\d{2}-\d{2}\s*이후/g,   // 열린 기간 — 늙지 않는다(COUNT_ROTS 가 만든 꼴)
    ];
    let bare = head;
    for (const s of SAFE) bare = bare.replace(s, ' ');
    /* 수 — 일반적으로. 한글은 십단위 단독(마흔)과 십단위+낱수(스물세) 둘 다 받는다 */
    const KO = '(?:(?:열|스물|서른|마흔|쉰|예순|일흔|여든|아흔|백|천)\\s*(?:하나|한|둘|두|셋|세|넷|네|다섯|여섯|일곱|여덟|아홉)?'
    ...
    const RO = '(?:[MDCLXVI]{2,})';
    const NUM = `(?:\\d+|${KO}|${EN}|${RO})`;
    /* 세는 이름 — 「목록의 한 항목」을 부르는 말. 뜻이 정해져 있어 목록이어도 안 샌다 */
    const UNIT = '(?:줄|항목|가지|개|건|교훈|규칙|items?|entries|lines?|lessons?|rules?)';
    const hits = [];
    /* ① 수 → 세는 이름 (「열두 줄」·「17항목」·「열일곱 개의 항목」·「17가지 교훈」) */
    for (const m of bare.matchAll(new RegExp(`${NUM}\\s*(?:개(?:의)?\\s*)?${UNIT}`, 'gi'))) {
      if (m[0].trim()) hits.push(`개수를 적었다: 「${m[0].trim()}」`);
    }
    /* ② 세는 이름 → 수 (「항목 17개」). 사이는 공백뿐 — 벌어지면 세는 말이 아니라 문장이다 */
    for (const m of bare.matchAll(new RegExp(`${UNIT}\\s*${NUM}\\s*${UNIT}?`, 'gi'))) {
      if (m[0].trim()) hits.push(`개수를 적었다: 「${m[0].trim()}」`);
    }
    /* ③ 항목 번호 범위 — 「★1~★15」·「1~15번」. 세는 이름이 없어 ①②에 안 걸리는데
       **항목이 늘면 그대로 늙는다**(★16 을 쓰는 날 「★1~★15」는 거짓이 된다).
       ★말뜻을 몰라도 잡히는 유일한 무단위 꼴이라 여기만 따로 판다.
         나머지 무단위 꼴(「지금까지 열일곱」·「(총 17)」)은 못 잡는다 — 초록 줄에 그렇게 적는다. */
    for (const m of bare.matchAll(/★?\d+\s*[~－–—-]\s*★?\d+\s*(?:번|항목|줄)?/g)) {
      if (/★/.test(m[0]) || /(?:번|항목|줄)/.test(m[0])) hits.push(`항목 번호 범위를 적었다: 「${m[0].trim()}」`);
    }
    /* ④ 닫힌 기간 — 「08-09~10」·「이틀 동안」. 열린 꼴(「08-08 이후」)만 SAFE 로 빠진다 */
    if (/\d{1,2}-\d{1,2}\s*~/.test(head)) hits.push('기간을 닫힌 꼴로 적었다: 「날짜~날짜」');
    for (const m of bare.matchAll(new RegExp(`(?:이틀|사흘|나흘|${NUM}\\s*일)\\s*(?:동안|간)`, 'g'))) {
      hits.push(`기간을 적었다: 「${m[0].trim()}」`);
    }
    if (hits.length) {
      /* ★[SAY_BOTH 2026-08-11] 붉을 때 **두 갈래를 다 적는다.**
         이 그물은 「수 + 세는 이름」의 꼴을 볼 뿐, 그 수가 **이 목록을 센 것인지**는 모른다.
         실측 헛붉음 둘 — 「세 가지 관점에서 본다」·「한 줄로 요약하면」. 둘 다 멀쩡한 글이다.
         ★그런데 옛 안내문은 「세지 않아도 참인 문장으로 바꿀 것」 하나만 말했다.
           멀쩡한 글에까지 그 지시가 나가면 **틀린 지시**가 되고, 몇 번 반복되면
           사람이 이 붉은색을 무시하기 시작한다(★9 늑대). 그래서 갈래를 열어 준다. */
      no(`${DOC} 머리말에 「수 + 세는 이름」이 있다 — 목록을 센 수라면 항목이 늘면 늙는다\n    `
        + [...new Set(hits)].join('\n    ')
        + '\n    → 둘 중 하나다.'
        + '\n      ① 목록을 센 수다 → 고치지 말고 **세지 않아도 참인 문장**으로 바꿀 것.'
        + '\n         개수는 목차가, 기간은 각 항목이 이미 말한다(COUNT_ROTS).'
        + '\n      ② 목록과 무관한 수다(예: 「세 가지 관점」·「한 줄로 요약」)'
        + '\n         → 이 검사의 SAFE 에 그 꼴을 한 줄 더하고 **왜 안 늙는지**를 옆에 적을 것.'
        + '\n    ※ 이 그물은 꼴만 본다. 그 수가 이 목록을 센 것인지는 사람이 정한다.');
    } else {
      const n = (raw.match(/^\*\*\d+[-a-z]*\. /gm) || []).length + (raw.match(/^## ★\d+\. /gm) || []).length;
      ok(`${DOC} 머리말이 스스로를 세지 않는다 (지금 항목 ${n} — 이 수는 여기서만 세고 어디에도 안 적는다)`);
      /* ★★[NET_SAYS_GAPS 2026-08-11] **초록일 때 안 본 것을 말한다.**
         이 그물은 「수 + 세는 이름」이 붙은 꼴만 본다. 세는 이름 없이 세는 말은 못 잡는다 —
         코워크가 다섯 발을 쏴서 전부 샜다(실측): 「지금까지 열일곱」·「(총 17)」·
         「1번부터 17번까지」·「★15 까지」. (「★1~★15」 범위 꼴만 아래에서 따로 잡는다)
         ★그걸 다 잡으려고 「수가 나오면 전부 붉게」로 가면 「쓸모는 하나다」까지 붉어진다.
           이미 시도했고 되물렸다. 남은 구멍은 **말뜻을 알아야** 메워지고, 그건 사람 몫이다.
         ★그러면 이 줄이 필요하다 — 안 본 것을 말하지 않으면 사람이 초록을 커버리지로 읽는다.
           목록 ★4 가 그 병이고, 이 검사가 지키는 문서가 바로 그 목록이다. */
      console.log('    ☐ 이 그물이 안 보는 것 — 세는 이름 없이 세는 말('
        + '「지금까지 열일곱」·「(총 17)」·「1번부터 17번까지」). 초록은 커버리지가 아니다.');
    }
  }
}
/* ★★[STOP_HERE 2026-08-11 · 두 세션 합의] 이 7) 블록은 **여기서 멈춘다.** 더 조이지 말 것.
   COUNT_ROTS → DOC_SELF_COUNT → COUNT_NET_OPEN → HEAD_BOUNDED 로 네 번 강화했다.
   ★멈추는 기준을 적어 둔다(다음에 또 손대고 싶어질 때 이 줄을 먼저 볼 것) —
     **막는 결함의 값 < 헛붉음이 치를 값**이 되면 멈춘다.
     여기 결함의 값은 「문서 머리말 문장 하나가 낡음」이다. 작다.
     반면 헛붉음의 값은 「사람이 이 붉은색을 무시하기 시작함」이고, 그건 이 검사만이 아니라
     **같은 게이트의 다른 검사까지** 함께 무시하게 만든다(★9 늑대). 값이 번진다.
   ★비용은 이미 두 번 실제로 나왔다 — 넓힌 판이 헛붉음 아홉 건을 냈고(HEAD_BOUNDED),
     조인 판을 고치다 문법 오류를 「다 잡았다」로 읽을 뻔했다(COUNT_NET_OPEN).
   ★남은 구멍은 **말뜻을 알아야** 메워진다(「지금까지 열일곱」이 목록을 센 수인지 아닌지).
