   ...
   ───────────────────────────────────────────────────────────────── */
{
  ...
  if (!fs.existsSync(p)) no(`${DOC} 가 없다 — 교훈 목록이 사라지면 이 검사도 뜻이 없다`);
    ...
    /* ★수사는 **긴 것부터** 늘어놓는다 — 「열다섯」이 「열」에 먼저 먹히면 뒤가 안 맞는다.
       ★뒤에 오는 조사·서술을 막지 않는다. 처음엔 `(?![가-힣])` 를 붙였다가
         「17항목**이**다」·「열다섯 가지**다**」를 놓쳤다(돌연변이 ⑤⑥ 실측).
         한국어에서 그 자리는 거의 늘 조사·서술이 붙는다 — 막으면 진짜만 새어 나간다. */
    const NUM = ['[0-9]+', '열하나', '열한', '열둘', '열두', '열셋', '열세', '열넷', '열네',
      '열다섯', '열여섯', '열일곱', '열여덟', '열아홉', '스물', '서른',
      '하나', '다섯', '여섯', '일곱', '여덟', '아홉', '한', '두', '세', '네', '열'].join('|');
    const hits = [];
    /* ① 목록 전체를 센 수 — 「열두 줄」·「17항목」·「열다섯 가지」 */
    for (const m of head.matchAll(new RegExp(`(${NUM})\\s*(개\\s*)?(줄|항목|가지)`, 'g'))) {
      hits.push(`개수를 적었다: 「${m[0].trim()}」`);
    }
    /* ② 닫힌 기간 — 「08-09~10」·「이틀 동안」. 열린 꼴(「08-08 이후」)은 늙지 않는다 */
    if (/\d{1,2}-\d{1,2}\s*~/.test(head)) hits.push('기간을 닫힌 꼴로 적었다: 「날짜~날짜」');
    for (const m of head.matchAll(/(이틀|사흘|나흘|\d+\s*일)\s*(동안|간)/g)) {
      hits.push(`기간을 적었다: 「${m[0].trim()}」`);
    }
    if (hits.length) {
      no(`${DOC} 머리말이 스스로를 세고 있다 — 그 수는 항목이 늘면 늙는다\n    `
        + [...new Set(hits)].join('\n    ')
        + '\n    → 수를 고치지 말고 **세지 않아도 참인 문장**으로 바꿀 것.'
