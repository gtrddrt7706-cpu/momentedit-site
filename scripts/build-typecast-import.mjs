...
for (let i = 0; i < lines.length; i++) {
  ...
  clips.push({ no: m[1], id: m[2], label: m[3], file: m[4], paras });
}
if (clips.length !== 54) { console.error(`✗ 클립 수 불일치: ${clips.length} (기대 54)`); process.exit(1); }

// ── 조립
fs.mkdirSync(OUT, { recursive: true });
const manifest = { version: 1, source: '더빙_녹음_대본_최종.txt', gap: GAP, parts: [], clips: [] };
let totalSent = 0;

for (const P of PARTS) {
  const mine = clips.filter((c) => P.has(c.id));
  const body = [];
  let sentInPart = 0;

  for (const c of mine) {
    const role = ROLE_OF(c.id);
    const sents = [];
    ...
    const rec = { no: c.no, id: c.id, label: c.label, file: c.file, role,
                  part: P.f, head: GAP.head, tail: over.tail ?? GAP.tail, sents: [] };
  ...
  manifest.parts.push({ file: P.f, title: P.t, role: P.role, clips: mine.length, sents: sentInPart, chars });
...
R.push(`## 무엇이 들어 있나 — ${clips.length}클립 · ${totalSent}문장 · ${PARTS.length}파트`, '');
R.push('| 파일 | 내용 | 화자 | 클립 | 문장 | 글자 |');
R.push('|---|---|---|---|---|---|');
for (const p of manifest.parts) R.push(`| \`${p.file}\` | ${p.title} | ${p.role} | ${p.clips} | ${p.sents} | ${p.chars.toLocaleString()} |`);
...
R.push('3. **화자 감지** — `안내`/`진행`/`편지` 중 그 파트의 화자 하나만 잡히면 정상입니다.');
R.push('4. **보이스 배정** — 그 화자에 목소리를 고릅니다. 파트마다 다른 목소리를 배정하는 게 설계입니다.');
...
R.push('7. 파트별 폴더에 풀고 `node scripts/assemble-narration.mjs`를 돌립니다. 문장이 클립으로 붙고, 여백이 들어가고, 음량이 맞춰집니다.', '');
...
{
  const dup = {};
  for (const c of manifest.clips) for (const s of c.sents) (dup[s.text] ||= []).push(c.no);
  ...
  if (rep.length) {
    ...
    R.push('**① 같은 문장은 같은 감정·속도로 맞추세요.** 클립 05의 "신랑 신부, 입장!"과 클립 09의 그것이 다르게 들리면,');
...
R.push('아직 A/B가 안 끝났으므로 **지금은 역할명이 기본값**입니다.', '');
fs.writeFileSync(path.join(OUT, 'README.md'), R.join('\n') + '\n', 'utf8');

console.log(`\n✓ ${clips.length}클립 · ${totalSent}문장 → ${OUT.replace(root + '/', '')}/`);
