...
const GAP = {
  head: 0.35,     // 클립 앞 무음 0.3~0.4
  sent: 0.9,      // 문장 사이 기본 0.8~1.2   (구 1.8 — 실청에서 너무 길었다)
  tail: 0.9,      // 클립 뒤 무음 0.8~1.0
};
...
for (const P of PARTS) {
  ...
  for (const c of mine) {
    ...
    sents.forEach((s, k) => {
      ...
      if (s.text === DECL_LINE) { before = 0.5; if (!last) after = 0.8; }
      if (s.text === DEMO_LINE) { before = 0.5; if (!last) after = 0.5; }
      // 혼주 편지 소제목(하나~넷) 앞뒤 각 1.1초 — 없으면 3분짜리 편지의 구조가 귀에 안 들린다
      // (구 1.5 · GAP_SHRUNK로 함께 줄였다 — 가장자리 무음을 깎고 나니 1.5는 뚝 끊긴 것처럼 들린다)
      if (c.id === 'G10' && /^(하나|둘|셋|넷),\s/.test(s.text)) { before = 1.1; if (!last) after = 1.1; }
      // 문단 경계는 한 박 더 쉰다
      // 마지막 줄(서명)은 새 문단을 여는 게 아니라 앞줄을 닫는 코다다 — 문단 들머리 여백을 주지 않는다
      else if (c.id === 'G10' && s.first && k > 0 && !last) { before = Math.max(before, 0.5); }
...
R.push('| 자리 | 초 | 근거 |');
R.push('|---|---|---|');
R.push(`| 클립 앞 | ${GAP.head} | 대본 공통 규격 0.3~0.4 |`);
R.push(`| 문장 사이 | ${GAP.sent} | 대본 공통 규격 0.8~1.2 · ★타입캐스트가 넣어 주는 앞뒤 무음은 조립기가 깎아 냅니다(GAP_SHRUNK) |`);
R.push(`| 클립 뒤 | ${GAP.tail} | 대본 공통 규격 0.8~1.0 |`);
R.push('| `…위하여!`로 끝나는 클립 뒤 | 4.5 | 축배 뒤 환호가 터진 자리의 낙차 (G9-toast·G9-both 둘 다) |');
R.push('| 폐식(`narr-close`) 뒤 | 2.5 | 예식의 마지막 소리 · 여운 |');
R.push(`| \`${DECL_LINE}\` 앞/뒤 | 0.5 / 0.8 | ★DECL_PAUSE_POS |`);
R.push(`| \`${DEMO_LINE}\` 앞/뒤 | 0.5 / 0.5 | W2-b 답 시연 구간 |`);
R.push('| 혼주 편지 소제목(하나~넷) 앞/뒤 | 1.1 / 1.1 | 3분 편지의 구조를 귀로 듣게 |');
