...
const DEFAULT_VOICE = {
  ...
  //     24·25(합창 재료)를 다시 받아 build-chorus.mjs 로 26을 다시 겹쳐야 한다.
  // ★★[VOICE_GROOM_3 2026-09-26 사장님] 신랑 이겸 → 진한(타입캐스트 캐릭터 이름).
  //   날 wav 음높이 중앙은 둘 다 135.6Hz 근처 — [VOICE_GROOM_2] 처럼 «음높이로 바뀐 것이 재진다»는 여기선 성립하지 않는다.
  //   음색(MFCC 평균)으로는 떨어진다(코워크 실측: 진한 ↔ 옛 이겸 중심 거리 2.6 · 한 목소리 안 흩어짐 1.5 안팎) — clip-voice-id 로 다시 잰다.
  //   ★순서: 이 줄을 먼저 바꾸고 그다음 들인다 — 들일 때 · 조립할 때 성우표에서 이름을 가져가 _index.json · _recorded.json 에 적는다.
  신랑: '진한',
  // ★★[VOICE_BRIDE_2 2026-09-26 사장님] 신부 서진 → 예슬(타입캐스트 캐릭터 이름). 날 wav 음높이 중앙 228.6Hz(코워크 · voice-gap 방법) —
  //   같은 글 «자리에 앉아 주시면 됩니다.» 옛 서진 176Hz → 예슬 215Hz. 녹음하며 두 줄을 고치셨다(원천 배역_예시_대사.txt · [BRIDE_TEXT_2]).
  신부: '예슬',
  // ★[VOICE_FATHER_2 2026-09-26 사장님] 아버님 권일 → 대진(18줄 도착). 음높이 중앙 114.3Hz(옛 권일과 같은 값 · 코워크 voice-gap 방법) —
  //   신랑 진한(133Hz)과 19Hz 떨어져 [VOICE_GAP] 규칙(15Hz)은 넘는다. 녹음하며 한 줄을 고치셨다([FATHER_TEXT_2] «싸워도 좋은데,»).
  아버님: '대진',
  // ★[VOICE_MOTHER_2 2026-09-26 사장님] 어머님 주하 → 연화. 아래 [VOICE_STEPMOM] 주석의 «연화는 영구 탈락»(8월 조사 · 패러디 밈 리스크)을
  //   사장님이 알고 고르셨다 · 어머님 덕담(13_bless-mother)은 미리듣기 전용(당일 콘솔은 틀지 않는다). 대본도 새로 · [MOTHER_TEXT_2] 중간판 15문장.
  어머님: '연화',
  // ★[VOICE_FRIEND_2 2026-08-21] 하객대표 영목 → 규민
  //   왜 — 사용자 판정: 15_toast *"너무 짧고 어색해 · 더빙 성우 바꿔야해"*.
  ...
  /* ★★[TOAST_NONE 2026-09-20 사장님 확정 「친구부분멘트 아예 삭제」] 하객대표 자리가 없어졌다.
     위 [VOICE_GAP] 이 「규민 148 vs 우성 151 — 사실상 같은 목소리」라 교체 후보를 찾던 중이었는데,
     **자리 자체가 사라져** 그 숙제가 함께 닫혔다. 87_narr-toast-none 이 그 자리를 말로 닫는다.
     ★배정은 지운다 — 안 쓰는 이름을 남겨 두면 생성기가 「실제 역할에 없다」고 잡는다(실제로 잡혔다).
     ★위 주석은 지우지 않는다 — 되살릴 때 필요한 근거가 거기 다 있다(F0 실측·교체 후보 세진). */
  ...
  //     ★2026-09-26 사장님 · 어머님 = 연화 · 비추천 이력을 알고 고르심 · 미리듣기 전용([VOICE_MOTHER_2]). 위 줄은 그때 판정이라 지우지 않는다.
  ...
  // ★[STEPMOM_2 2026-09-26 사장님] 시어머님 성우도 바꾼다 — 예슬(신부)과 정숙이 음높이가 같고(222Hz) 음색도 서로 가장 닮았다(clip-voice-id 0.95).
...
const ROLE_OF = (id) => {
  ...
  /* ★★[VOICE_44_HOST 2026-09-20] N0(44 단체촬영 개시 신호)만 «진행»으로 뺀다.
     그룹은 「폐식 후 브릿지」라 안내인데, **큐 차례로는 사진 구간의 «첫 줄»**이다.
       18 26 진행 · 19 **44 안내** · 20~24 60·61·63·64·65 진행 · 25 45 안내 · 26 47 안내
     안내 목소리가 한 문장 나왔다가 **여섯 클립 뒤에** 돌아온다. 코워크가 눈으로 찾았다.
     ★그리고 44 시점엔 예식이 안 끝났다 — 「오늘의 예식은 여기까지입니다」(45)가 여섯 뒤다.
       안내 목소리는 예식 «밖»을 맡는데 44 는 아직 안이다.
     ★코워크의 다른 안(44 를 65 뒤로)은 안 된다 — 「여기서부터는 사진입니다」가
       사진이 끝난 뒤(65)에 나오면 모순이다.
     ★지금 바꾸면 비용 0이다 — 44 는 문면이 바뀌어 재녹음 대기 중이라 아직 안 뽑았다.
     ★그룹 자체(N→G13)를 옮기지 않은 이유: 대본 번호·파트 파일이 함께 움직여 여파가 넓다.
       여기 한 줄이 «누가 읽나»의 유일한 원천이므로, 예외를 이 자리에 적는 것이 가장 좁다. */
  /* ★★[N_HOST 2026-09-20] N 그룹은 「폐식 후 브릿지」라 기본이 «안내»인데, **그 안에 진행이 섞였다.**
     한 줄 예외(N0)로 시작했다가 15n(N9)을 넣자 같은 자리에서 또 걸렸다 — 내가 만든
     [VOICE_RUNS] 가 「혼자 끼어든다」로 잡아 줬다. 예외가 둘이면 목록으로 둔다.
       N0  44 단체촬영 개시 — 큐 차례로 사진 구간의 첫 줄(60~65 가 진행이다)
       N9  87 축사 없음 안내 — 축배 «앞»이라 앞뒤가 전부 진행이다
     ★다음에 N 을 늘릴 때는 «큐 차례에서 앞뒤가 누구인가»를 먼저 보고 여기 넣을지 정한다.
       그룹 이름(N=브릿지)이 아니라 **실제 이웃**이 목소리를 정한다. */
  /* ★★[N_HOST_OPEN 2026-09-25] 새 코스 줄 N13~N19 는 예식 «안»이다 — 화촉(입장 앞) · 박수 선언 · 와인 · 준비한 순서 · 큰절 · 선창.
...
const PARTS = [
  ...
  { f: '6_예식뒤.txt', t: '닫는 말 · 다 함께 · 골라 트는 판', role: '진행',
    // ★[VOICE_44_HOST] N0(44) 이 여기로 온다 — 큐 차례로 G13(60·61·63·64·65) 바로 앞이라
    //   한 파일에 같은 목소리로 모인다. 「안 들어가는 클립」 검사가 이 자리를 요구했다.
    has: (id) => /^G1[234]-/.test(id) || /^N(0|9)$/.test(id) },   // [N_HOST]
...
/* ★★[TOAST_NONE 2026-09-20 사장님 확정] R-toast(하객대표 축배사 7문장)도 뺀다.
   사장님 지시 *「친구부분멘트 아예 삭제」* — 보류가 아니라 **폐지**다.
   ★그런데 «빼는 방법»은 보류와 같다 — 문안을 원천에서 지우지 않는다.
...
const GAP = {
  /* ★[HEAD_LEADIN 2026-08-09] 클립 앞 무음 0.18 → 0.4초. 사용자 제보:
     *"재생 누르면 처음 한마디 정도가 씹히는 증상"*. 실측하니 mp3 앞 무음이 0.23~0.30초뿐이었다 —
     브라우저가 재생을 시작해 소리가 실제로 나오기까지의 짧은 램프가 그 안에서 끝나지 않으면
     첫 음절이 잘려 들린다. 여백을 늘려 램프가 **무음 위에서** 끝나게 한다.
     ★0.18 은 '턴이 길다'를 잡느라 줄였던 값인데, 그건 **문장 사이**가 문제였지 클립 맨 앞이 아니었다.
       맨 앞 0.22초는 늘려도 늘어진 느낌을 주지 않는다(다음 소리가 이어지는 자리가 아니다). */
  head: 0.4,      // 클립 앞 무음   (원 0.4 → 1차 0.35 → 2차 0.18 → 2026-08-09 0.4 복귀)
...
// ★★[DECL_DEAD 2026-09-20] 이 상수가 **사문화돼 있었다.** [PERF_CANON_2] 가 호명 「신랑 신부,」를
//   빼면서 문면이 「이제 두 사람은 부부입니다.」가 됐는데 여기는 옛 문장이 그대로 남았다.
//   그래서 `s.text === DECL_LINE` 이 **한 번도 참이 되지 않았고**, 2026-08-17 이후 선언문 앞뒤
//   무음이 통째로 안 들어갔다. manifest 실측이 그것을 보여 준다 — 전 문장이 after=0.45 로 균일했다.
//   ★사장님이 실청에서 「다다다」라고 하신 것의 «소리 쪽» 원인이 이것이다(문면이 아니라 간격).
//   ★게이트도 공범이었다 — chk 가 마커 «문자열»만 보고 무음이 실제로 얹히는지는 안 봤다.
//     그래서 scripts/audit/decl-pause.mjs 를 만들어 manifest 를 실제로 재게 했다.
const DECL_LINE = '이제 두 사람은 부부입니다.';
...
const parse = (file) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(/^\[(\d{2,3})\]\s+(\S+)\s+·\s+(.+?)\s+→\s+\d{2,3}_(.+)\.mp3\s*$/);   // [PAD3]
    if (!m) continue;
    const paras = [];
...
/* ★[CLIP_COUNT] 클립 수는 손으로 적는다 — 일부러 그렇게 둔다. 대본이 조용히 늘거나 줄면
   ...
   ★고칠 때는 **왜 바뀌었는지**를 여기 남길 것. 숫자만 올리면 다음 사람이 같은 함정에 빠진다.
     ...
     82 → 84 [PHOTO_ASK 2026-08-16 사용자 확정] 하객에게 사진을 부탁하는 두 자리.
              사용자 원문: *"추천대로 진행해 우선 내가 실청테스트하면서 거를거니깐"*
              narr-photo-ask  : 예식 직전(04_guest-4-1min 뒤) — 04 의 마지막이 「휴대폰은 진동으로」라
                                「그리고」로 이어 붙였다. 소리를 줄이는 것과 사진을 남기는 것은 다른 부탁이다.
              narr-photo-send : 배웅(45_end-1a-farewell 뒤) — 단체촬영·인사 사진이 다 끝난 뒤라
                                그때가 하객 손에 사진이 가장 많다.
     ...
     85 → 86 [TOAST_NONE 2026-09-20 사장님 확정 「첫째 안」] 축사 없음 안내 1개(87_narr-toast-none).
              사장님 지시 *「친구부분멘트 아예 삭제」* 로 배역 15_toast(하객대표 7문장)를 폐지하면서,
              그 자리를 «빠진 것»이 아니라 «그렇게 정한 것»으로 닫는 한 줄을 세웠다.
              ★나레이션은 하나 늘고 배역은 하나 줄어 전체 클립은 109 → 108 이다.
                두 숫자가 반대로 움직이니 «전체가 그대로»라고 착각하지 말 것.
              ★번호 87 — FILES 맨 끝이다. 중간에 끼우면 86개가 전부 개명된다(2026-08-11 실측 사고).
     86 → 85 [ENTRY_OUT_B_DROP 2026-09-20] 79_narr-entry-out-B 를 RETIRED 에 넣어 하나 줄었다.
              ★★같은 커밋에서 **하나 늘고 하나 줄어 85 로 돌아왔다.** 가드에 한 번 걸려서야 알았다 —
     ...
     94 → 102 [NAR_0925 2026-09-25 코워크 회신 둘째 판] 새 줄 8개(100~107).
...
if (held.length) console.log(`  붙여넣기 제외 ${held.length}클립: ${held.map((h) => h.id).join(' · ')}`
  + `  (R-declare-* 은 기획서 §6 결정 7 «대기» · R-toast 는 2026-09-20 «폐지» — 성격이 다르다)`);
