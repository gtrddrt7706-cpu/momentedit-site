...
for (const P of PARTS) {
  ...
  const head = [
    ...
    `# 이 주석 줄(#)은 지우지 않아도 됩니다. 화자로 잡히지 않습니다.`,
    `#   ★그래도 표에 없는 화자가 하나 더 잡히면 그 칩의 ⊗ 를 눌러 제외하세요. 결과는 같습니다.`,
    spk.length === 1
  ...
  // ★COMMENT_NO_COLON — 주석 줄에 콜론이 있으면 출력하지 않는다.
  //   타입캐스트 화자 감지는 '#'를 주석으로 취급하지 않는다. 콜론 앞뒤만 본다.
  //   2026-08-01 실사고: `'화자: 대사' 형식이 아니라 화자로 잡히지 않습니다`라고 안내하던
  //   그 문장이 문장 안에 콜론을 품고 있어서, 안내문이 정확히 자기가 부정하던 일을 당했다.
  //   사용자 화면에 유령 화자 1개(대사 1개 · 줄 3)가 떴다. 5번 배역 파트엔 지뢰가 둘이었다.
  //   그래서 문구를 고치는 데서 멈추지 않고 여기서 막는다 — 사람이 다시 쓰면 다시 걸린다.
  const colonInComment = head.filter((l) => /^#/.test(l) && /[:：]/.test(l));
  if (colonInComment.length) {
    console.error(`\n✗ ${P.f} 머리 주석에 콜론이 있습니다 — 타입캐스트가 이 줄을 화자로 오검출합니다.`);
    for (const l of colonInComment) console.error(`   ${l}`);
    console.error('  주석에서 콜론을 빼세요(연결은 · 또는 —). 본문 대사 줄의 콜론은 정상입니다.\n');
    process.exit(1);
  }
...
R.push('   **표에 없는 화자가 하나 더 잡혔다면 그 칩의 `⊗`를 눌러 제외하세요.** 타입캐스트는 `#` 주석을 모릅니다 —');
R.push('   콜론 앞뒤만 보고 화자를 가르기 때문에, 주석에 콜론이 하나라도 있으면 그 줄이 유령 화자가 됩니다.');
R.push('   지금 파일은 주석에서 콜론을 전부 뺐고 생성기가 재발을 막지만(`COMMENT_NO_COLON`), 확인은 3초입니다.');
