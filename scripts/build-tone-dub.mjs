// 어조 41벌의 붙여넣기 대본을 뽑는다 [TONE_DUB]
//
// ★★[TONE_CULL 2026-09-20 사장님 「겹치거나 별로인 거 전부 삭제해 과감하게 갯수 상관없이」]
//   60 → **41**. TONE 이 38 에서 19 로 줄었다(NARV 22 는 그대로).
...
//   어조 41벌은 아직 manifest 에 없다(엔진 배선·생성기 재실행이 끝난 뒤에 들어온다).
...
/* ★41 = TONE 19 + NARV 22 ([TONE_CULL] 뒤). **사이 순서(valley)는 넣지 않는다.**
...
if (rows.length !== 41) no(`문안이 ${rows.length}벌이다 — TONE 19 + NARV 22 = 41 여야 한다`);
...
const list = [
  /* ★첫 줄이 스스로 버전을 말한다 [TONE_DUB_SELFID]
     ...
     이름은 `어조_명단.txt` 로 고정이고, 안이 19벌인지 41벌인지는 열어야 안다.
