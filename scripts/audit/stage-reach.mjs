/* ★[STAGE_REACH 2026-08-17 사용자 지시 "이런버그는 운영하는데 나오면 너무 취약하니깐 상세하게 각각
   ...
   ── 지금 이 검사는 «붉게» 끝난다 (2026-08-17 · 일부러 그렇게 둔다)
   남은 지적은 하나뿐이다 — 「후기」 단계로 올려 주는 동작이 없다.
   고치려면 «후기»가 무슨 뜻인지부터 정해야 해서(끝난 상태냐 · 기다리는 구간이냐)
   `나중에할일_체크리스트.md` 의 결정 대기함에 올려 두었다. 사용자가 고르면 한 줄로 배선된다.
   ★그때까지 이 붉음을 «통과»로 바꾸지 않는다. 아는 문제를 초록으로 덮으면 그 옆의 새 문제도 같이 덮인다.
   ★그래서 CI(merge-guard)에는 걸지 않는다 — 늘 붉은 게이트는 아무도 안 본다. 손으로 돌리는 검사다.
     결정이 배선되는 순간 실패 0건이 되고, 그때 CI 에 걸면 된다.
*/
