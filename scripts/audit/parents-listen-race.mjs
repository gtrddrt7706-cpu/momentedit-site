...
(function(){
  var T0 = Date.now();
  var S = window.__snd = { log: [], tts:false, audio:false, overlap:false, speaks:0, plays:0, cancels:0 };
  function mark(what){
  ...
  function FakeAudio(src){ this.src=src||''; this.paused=true; this.preload=''; this.currentTime=0; this.ended=false; this._h={}; }
  ...
  FakeAudio.prototype.play=function(){ S.plays++; this.paused=false; S.audio=true; mark('audio.play'); return Promise.resolve(); };
...
await br.close();
stop();
console.log(bad ? `\n✗ ${bad}건 실패 — 「듣기」가 두 소리를 동시에 낼 수 있습니다` : '\n✓ 네 상황 모두 통과 — 한 번에 한 소리만 납니다');
