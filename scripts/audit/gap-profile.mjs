...
//
// ★★[GAP_TRUTH 2026-08-14 코워크 실측] **manifest 의 지시값과 실측이 다르다. 대조는 실측으로 한다.**
//   manifest.gap 은 head 0.4 / tail 0.45 라고 **지시**하지만, 실제로 뽑혀 온 82클립을 재면
//   앞 중앙값 **0.23** · 뒤 중앙값 **0.50** 이다(코워크 ffprobe 6.1.1 실측 · 이 컨테이너엔 ffprobe 가
//   없어 여기서는 재현 못 함 · check-syl-rate 가 늘 rc 2 인 이유도 같다).
//   ★새 클립(어조 38벌)을 manifest 의 0.4 로 재면 **기존 82개가 전부 어긋난 것으로 나온다.**
//     그때 고칠 것은 소리가 아니라 자다. 지시값은 타입캐스트에 넣는 설정이고,
//     실측값은 그 설정이 실제로 만들어 낸 결과다 — 둘이 다른 것은 고장이 아니다.
//   ★이 숫자를 manifest 에 옮겨 적지 말 것. manifest 는 생성물이라 다음 생성 때 지시값으로 되돌아간다.
//
