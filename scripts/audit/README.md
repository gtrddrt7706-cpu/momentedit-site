...
- **★축2는 끄기 축이 아니라 상태 축이다 (2026-07-28에 고침).** 초판(2026-07-27)은 축2를 **끄는 방향으로만**
  ...
  **밸리를 켠 조합이 0개**. `S.toast`는 아예 설정되지 않아 전 조합 `undefined`였다.
  ...
  `chk` 마커 셋 다 초록으로 통과했다. 지금은 `STATES`(`ring`·`bless`·`valley`·`toast`)의 곱집합을 돈다.
  (`veil`은 2026-08-03 폐지 — 전 예식 동시입장이라 실행 불가. 축에서 내렸다.)
...
- **[4] 변이 검출** — 엔진을 건드리면 반드시 티가 나고(양성), 엔진 밖을 건드리면 반드시 조용해야(음성) 한다.
  ...
  ★음성 대조에 "아무 순위도 넘지 않는 값 변경"(예: `entry 10→9`)을 넣는다. **`RANK`는 서수다** —
...
(estMin 216 · prep 1,536 · \_mm 18 · \_cakeDup 9 · 블로커 8 × 5코스 × seq 프로파일 2종 = 17,870).
※ 2026-08-03 베일 폐지로 estMin·prep·\_mm 세 가드에서 `veil` 축이 빠져 70,970 → 17,870이 됐다.

**색인은 함수가 아니라 필드다.** 같은 필드를 같은 방향으로 읽는 자리가 함수 경계를 넘어 둘 이상 있다 —
`estMin()`과 카드 칩을 그리는 지역 클로저 `_mm()`이 letter·ringwarm·toast를 똑같이 읽는다.
함수 단위로 잡으면 둘을 별개로 세고 '둘이 어긋났는가'라는 진짜 질문을 못 던진다.

- **[A] 도달** — 어떤 값을 넣어도 출력이 안 바뀌는 필드. 죽은 분기이거나 스캐너가 못 밟는 자리다.
  ★이 스캐너가 생긴 이유가 여기다. 순서 시뮬레이터는 `courseDefaults`로 네 필드만 세팅해
