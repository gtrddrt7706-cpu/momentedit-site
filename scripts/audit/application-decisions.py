...
검사 셋
  ...
  ②' NOTE — 본문이 안 바뀌는 결정(기관 선택·자료 조사)의 기록. 검사는 안 하되 남긴다 [NOTE_KIND]
  ...
  ④ 검토함 — 대표가 올린 지적이 조용히 사라지지 않는가 [REVIEW_GATE]
            ★하위 항목(12-1 · 12-2)도 한 줄로 센다 [SPLIT_FACTS] — 한 발언에 사실이
            여럿이면 쪼개 적어야 하나만 판정하고 나머지가 묻히는 일이 없다.
...
LIMIT = {'Q1': 100, 'Q2': 2000, 'Q3-1': 2000, 'Q3-2': 2000,
         'Q4-1': 2000, 'Q4-2': 2000, 'Q8': 2000, 'Q10': 100}
# ★Q4-2 는 2026-09-14 폼 실화면 캡처로 2,000 확정 — 제목 「(2,000자)」 + 카운터 「0 / 2000자」.
#   종전 1,000 은 이 게이트를 처음 만든 커밋에 «근거 없이» 들어온 값이었다.
#   우리 1차 자료 두 곳(질문지_백지 · 최종본 제출 점검표)은 처음부터 2,000 이라고 적고 있었다.
#   ★한도가 열렸다고 채우지 않는다 — 967자에 다섯 항목이 이미 구체적이다(코워크 C-48 동의).
def margin(lim):
...
def inbox_rows(path):
    ...
    for ln, raw in enumerate(io.open(path, encoding='utf-8'), 1):
        s = raw.strip()
        # ★하위 항목(12-1 · 12-2)도 읽는다 [SPLIT_FACTS 2026-09-13]
        #   대표 한 발언에 사실이 여럿이면 «검토 N» 한 줄로는 하나만 판정하고 나머지가 묻힌다.
        #   실사고: ⑫번에 ①작가 10~20만 ②보정 1장 5천 ③임차료 200만 세 가지가 있었는데
        #   내가 ②만 판정하고 ⑫ 전체를 «보류»로 내려 ①③이 함께 사라졌다. 대표가 물어서야 드러났다.
...
def main(submit=False):
    ...
    for ln, raw in enumerate(io.open(LEDGER, encoding='utf-8'), 1):
        ...
        if kind == 'MUST' and c == 0:
            ...
            # ★본문에 걸 문자열이 없는 결정도 있다 [NOTE_KIND 2026-09-13]
            #   기관 선택·자료 조사처럼 «신청서 본문이 안 바뀌는 완료»가 실제로 있었다.
            #   그것을 MUST 로 억지로 만들면 엉뚱한 문자열을 지키게 되고,
            #   상태를 «보류»로 두면 끝난 일이 영영 미처리로 남는다. 그래서 종류를 하나 더 둔다.
            #   ★남발 방지 — NOTE 는 «무엇» 칸이 비면 실패한다(아래 검사).
            if len(what) < 10:
                fail.append('NOTE %d행: 무엇을 했는지가 너무 짧다 — 기록이 아니라 빈칸이다' % ln)
