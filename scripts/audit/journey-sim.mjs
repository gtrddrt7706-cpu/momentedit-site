...
/* 되돌리기 전수 — 도달했던 단계에서 **그보다 앞선 모든 단계**로 강제 이동한 경우를 기계적으로 만든다.
   ★손으로 목록을 적으면 빠뜨린 조합이 남는다(그 빠뜨린 칸이 [PAID_STAGE_BACK] 이었다). */
const ORDER = ['신청접수', '상담확정', '시착', '상담완료', '계약완료', '입금완료', '제작중', '예식완료', '결과물전달'];
const MATRIX = [];
for (let from = 4; from < ORDER.length; from++) {          // 계약완료 이후에 도달한 뒤
  for (let to = 0; to < from; to++) {                      // 그보다 앞선 모든 단계로 되돌림
    MATRIX.push({ at: `${ORDER[from]} → ${ORDER[to]}(돈 기록 유지)`,
      row: Object.assign({}, PAID_MORE, { 현재단계: ORDER[to] }) });
  }
}
...
try {
  ...
  const check = async (label, row) => {
    const { d, err, adm } = payloadAt(row);
    ...
    ok(r.cards.length > 0 || !!r.now, `${label} · 백지가 아니다`, JSON.stringify(r.cards) + ' now=' + r.now);
  ...
  console.log(`\n[되돌리기 전수 — 도달 단계 × 앞선 단계 ${MATRIX.length}가지]`);
  for (const m of MATRIX) await check(m.at, m.row);
