...
/* ★★[FIXTURE_NO_ROT 2026-08-18 점검에서 실측] 고정 날짜를 쓰지 않는다.
   종전엔 계약서발송일시가 '2026-08-16 10:00' 로 박혀 있었다. 서명 기한은 발송 +72시간이라,
   달력이 사흘만 지나면 같은 픽스처가 «발송됨»에서 «기한 지남»으로 **말없이 뜻이 바뀐다** —
   실제로 그렇게 바뀌어 붉어졌고, 그 붉음은 제품이 아니라 시간이 만든 것이었다.
   시간이 지나 색이 바뀌는 검사는 믿을 수 없다. 지금을 기준으로 상대 시각을 만든다.
   ★«기한 지남»을 보고 싶으면 그 픽스처를 따로 만든다(아래 '계약발송·기한지남'). */
const KST = (hoursAgo) => {
  const d = new Date(Date.now() - hoursAgo * 3600 * 1000 + 9 * 3600 * 1000);
  const p2 = (n) => ('0' + n).slice(-2);
  return d.getUTCFullYear() + '-' + p2(d.getUTCMonth() + 1) + '-' + p2(d.getUTCDate())
    + ' ' + p2(d.getUTCHours()) + ':' + p2(d.getUTCMinutes());
};
const SENT_OK = KST(2);       // 2시간 전 발송 — 아직 기한 안(72h)
const SENT_OLD = KST(80);     // 80시간 전 발송 — 기한 지남
const SIGNED = KST(1);        // 1시간 전 서명
...
const STOPS = [
  ...
  { at: '계약발송',   row: { 현재단계: '계약완료', 계약상태: '발송', 계약서발송일시: SENT_OK, 계약총액: AMT } },
  /* [FIXTURE_NO_ROT] 기한 지남은 별도 픽스처로 — «지금 할 일»이 재요청으로 바뀌는지 본다(NOW_CONTRACT_EXPIRED) */
  { at: '계약발송·기한지남', row: { 현재단계: '계약완료', 계약상태: '발송', 계약서발송일시: SENT_OLD, 계약총액: AMT } },
  { at: '서명완료·입금대기', row: { 현재단계: '계약완료', 계약상태: '서명완료', 계약서명일시: SIGNED, 계약총액: AMT, 입금상태: '대기' } },
  { at: '입금신고',   row: { 현재단계: '계약완료', 계약상태: '서명완료', 계약서명일시: SIGNED, 계약총액: AMT, 입금상태: '완료신호', 입금자명: '정희준' } },
  { at: '입금확인',   row: { 현재단계: '입금완료', 계약상태: '서명완료', 계약서명일시: SIGNED, 계약총액: AMT, 입금상태: '확인', 입금자명: '정희준' } },
...
const PAID_MORE = { 계약상태: '서명완료', 계약서명일시: SIGNED, 계약총액: AMT,
...
const FULL_LATE = { 신랑이름: '김희준', 신부이름: '이미쿠', 상품타입: '시그니처', 개인코드: 'ME-TEST',
  계약상태: '서명완료', 계약서명일시: SIGNED, 계약총액: AMT, 예식일: '2026-10-26',
  입금상태: '확인', 입금자명: '정희준', 중도금상태: '확인', 잔금상태: '확인',
...
const SNAP = ['계약완료', '입금완료', '촬영완료', '결과물전달'].map(function (st) {
  return { at: `웨딩스냅 ${st}`, row: { 현재단계: st, 상품타입: '웨딩스냅', 계약상태: '서명완료',
    계약서명일시: SIGNED, 계약총액: 1200000, 입금상태: (st === '계약완료' ? '대기' : '확인') } };
});
const BACKS = [
  { at: '입금확인 → 계약완료로 되돌림', row: { 현재단계: '계약완료', 계약상태: '서명완료', 계약서명일시: SIGNED, 계약총액: AMT, 입금상태: '확인', 입금자명: '정희준' } },
...
try {
  ...
  {
    const a = payloadAt({ 현재단계: '계약완료', 계약상태: '서명완료', 계약서명일시: SIGNED, 계약총액: AMT, 입금상태: '확인', 입금자명: '정희준' });
    let hq = [];
