...
for (const f of FILES) {
  /* ★[NO_SILENT_SKIP 2026-08-09] 목록에 적힌 파일이 없으면 **멈춘다.** 종전엔 조용히 건너뛰었다.
     이 목록은 merge-guard 가 손으로 넘겨 주는 17쪽이라, 파일 하나를 옮기거나 이름을 바꾸면
     그 쪽은 **검사에서 조용히 빠진다.** 그러고도 초록이라 아무도 모른다.
     ★코드 세션이 세 번째로 같은 병을 짚었다(유령 기록 · 대장을 믿던 판정 · 붙여넣기 파일 없음):
       **'없으면 통과'는 늘 조용한 거짓말이 된다.** 그래서 이 꼴을 볼 때마다 뒤집는다. */
  if (!fs.existsSync(f)) { console.error(`✗ ${f} 가 없습니다 — 목록이 낡았거나 파일이 옮겨졌습니다. 이대로 두면 이 쪽은 검사에서 조용히 빠집니다`); bad++; continue; }
