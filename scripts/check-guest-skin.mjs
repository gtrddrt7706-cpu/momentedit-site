...
//
// ★★[NO_GATE 2026-08-10] **이 검사는 어떤 게이트도 돌리지 않는다.**
//   화면을 고쳤으면 푸시 전에 손으로 돌린다. 0/1/2 는 사람이 읽는 값이다.
//   merge-guard 는 이 파일을 grep 으로 마커만 센다(merge-guard.sh 969행이 그렇게 적어 두었다).
//   ★「종료 코드 2 도 0 이 아니라 게이트가 막는다」는 말은 여기서 성립하지 않는다 —
//     막을 게이트가 애초에 없다. 없는 방벽을 근거로 안심하지 말 것.
//
    ...
    const ctlLeak = await p.evaluate(() => {
      ...
      /* 고객이 조작부에서 봐도 되는 것 — **이름을 적은 것만.** 넓히려면 여기 한 줄을 늘리고
         ...
         그 순간 앞으로 생길 디렉터 버튼이 전부 통과한다(이 검사가 막으려던 바로 그것).
           ...
           gPause: 잠시 멈춤·이어듣기 [GUEST_PAUSE 2026-08-15 사용자 지시] — 전화가 오면 설 자리
        ★gPause 를 빠뜨린 채 버튼만 화면에 올라가, 2026-08-15 이후 이 검사는 **줄곧 빨강**이었다
          (실측: 변경 전후 모두 `rect.[object SVGAnimatedString]` 2건으로 exit 1).
          빨강이 고정되면 검사는 아무것도 못 잡는다 — 새 누출이 생겨도 색이 안 바뀐다.
          ★고객 버튼을 새로 만들면 **같은 커밋에서** 이 줄에 이름과 이유를 함께 적을 것. */
      const OK = ['mBtn', 'gPrev', 'gPause'];
